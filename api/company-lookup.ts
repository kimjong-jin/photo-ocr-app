...
{"representative":"","phone":"","address":"","companyName":"","confidence":"높음|보통|낮음","note":""}`;
  ...
  for (const model of MODELS) {
    ...
    try {
      ...
      // AI(Gemini)가 프롬프트에서 이미 지역번호 일치를 검증하므로 그 결과를 신뢰(맡김).
      // 서버가 전화번호를 강제로 공란 처리하지 않는다 — 지역번호가 달라 보이면 '참고' 주의만 남김.
      if (phone && addrOut && !phoneRegionOk(addrOut, phone)) {
        note = `참고: 전화(${phone}) 지역번호가 주소 시·도와 달라 보임 — 본사/다른 지점일 수 있으니 확인 권장. ${note}`.trim();
      }
      const result = {
