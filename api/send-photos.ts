...
const MAX_TOTAL_BYTES = 3_200_000; // 3.2MB binary → base64 ~4.3MB → Vercel 4.5MB 한도 이내
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    if (!brevoRes.ok) {
      let errMsg = `Brevo 오류 (${brevoRes.status})`;
      try {
        const parsed = JSON.parse(text);
        // Brevo returns { code, message } or { error } or a plain string
        errMsg = parsed?.message || parsed?.error || parsed?.code || JSON.stringify(parsed);
      } catch {
        if (text && text.length < 400) errMsg = text;
      }
