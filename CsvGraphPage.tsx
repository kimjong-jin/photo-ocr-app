...
const CsvGraphPage: React.FC<CsvGraphPageProps> = ({ userName, jobs, setJobs, activeJobId, setActiveJobId, siteLocation, onDeleteJob, locationList = [] }) => {
  ...
  return (
    <div className="w-full max-w-3xl bg-slate-900/60 backdrop-blur-sm border border-slate-700/50 rounded-xl shadow-lg p-5 sm:p-7 space-y-5">
      <h2 className="text-sm font-semibold text-sky-400/70 tracking-widest uppercase mb-1">CSV 그래프 (P5)</h2>

      {jobs.length === 0 && (
        <p className="text-center text-slate-600 text-xs py-6">작업을 추가해 시작하세요.</p>
      )}
            ...
            {jobs.map(job => (
              ...
              >
                <div className="flex justify-between items-center">
                  <span className={`text-sm font-medium ${activeJobId === job.id ? 'text-sky-300' : 'text-slate-200'}`}>
