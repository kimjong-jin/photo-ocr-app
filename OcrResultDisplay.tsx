        ...
        {ocrData.length > 0 && (
            ...
            <table className={`min-w-full divide-y divide-slate-700 ${isManualEntryMode ? 'table-auto' : 'table-fixed'}`}>
                <colgroup>
                    {isManualEntryMode ? (
                        <>
                            ...
                            <col /> {/* Value 1 (auto width) */}
                            {showTwoValueColumns && <col />} {/* Value 2 (auto width) */}
                         ...
                         <>
                            ...
                            {showTwoValueColumns ? (
                                <>
                                    <col className="w-32" /> {/* Value 1 */}
                                    <col className="w-32" /> {/* Value 2 */}
                                </>
                            ) : (
                                <col className="w-32" /> /* Value */
                            )}
                            <col className="w-40" /> {/* Identifier 1 */}
                            {showTwoValueColumns && <col className="w-40" />} {/* Identifier 2 */}
                        </>
                    )}
                </colgroup>
                ...
                <tr>
                    ...
                    <th scope="col" className="px-3 py-3 text-center text-xs font-medium text-slate-300 uppercase tracking-wider">최종 저장 시간</th>
                    {showTwoValueColumns ? (
                    <>
                    ...
                    {!isManualEntryMode ? (
                        <>
                           <th scope="col" className="px-3 py-3 text-center text-xs font-medium text-slate-300 uppercase tracking-wider">{isTnTpMode ? 'TN 식별자' : '식별자'}</th>
                           {isTnTpMode && <th scope="col" className="px-3 py-3 text-center text-xs font-medium text-slate-300 uppercase tracking-wider">TP 식별자</th>}
                        </>
                    ) : (
                        <th scope="col" className="px-3 py-3 text-center text-xs font-medium text-slate-300 uppercase tracking-wider">구분</th>
                    )}
                ...
                {ocrData.map((entry, index) => {
                    ...
                    const isDividerRow = isManualEntryMode && entry.identifier === 'Z 2시간 시작 - 종료';
                    ...
                    if (isDividerRow) {
                        const colSpan = 2 + (showTwoValueColumns ? 2 : 1) + 1; // No, Time, Value(s), Identifier
                        return (
                            <tr key={entry.id}>
                    ...
                    let responseTimeData = { type: '', value: '' };
                    if (isResponseTimeRow && entry.value) {
                        try {
                            const parsed = JSON.parse(entry.value);
                            if (parsed.type && parsed.value !== undefined) {
                                responseTimeData = { type: parsed.type, value: String(parsed.value) };
                            }
                        } catch (e) { /* Not JSON, ignore */ }
                    }

                    const handleResponseTimeChange = (newType: 'seconds' | 'minutes' | 'length', newValue: string) => {
                        if (newValue.trim() === '') {
                            onEntryPrimaryValueChange(entry.id, '');
                        } else {
                            onEntryPrimaryValueChange(entry.id, JSON.stringify({ type: newType, value: newValue }));
                        }
                    };
                    ...
                    <tr key={entry.id} className={`${isSequenceRow ? 'bg-slate-900' : 'hover:bg-slate-700/30'} transition-colors duration-100`}>
                        <td className={`px-2 whitespace-nowrap text-sm text-slate-400 text-center ${isSequenceRow ? 'py-1' : 'py-2.5'}`}>{index + 1}</td>
                        <td className={`px-2 whitespace-nowrap ${isSequenceRow ? 'py-1' : 'py-2.5'}`}>
                        ...
                        {isResponseTimeRow ? (
                            <td colSpan={showTwoValueColumns ? 2 : 1} className={`px-2 whitespace-nowrap ${isSequenceRow ? 'py-1' : 'py-2.5'}`}>
                                <div className="flex items-center gap-2">
                                    {/* Seconds input */}
                                    <div className="flex-1 flex items-center gap-1.5">
                                        <input
                                            type="text"
                                            value={responseTimeData.type === 'seconds' ? responseTimeData.value : ''}
                                            onChange={(e) => handleResponseTimeChange('seconds', e.target.value)}
                                            onBlur={() => onEntryValueBlur?.(entry.id, 'primary')}
                                            disabled={responseTimeData.value !== '' && responseTimeData.type !== 'seconds'}
                                            className={`${baseInputClass} text-slate-200 text-right disabled:bg-slate-800 disabled:text-slate-500`}
                                            aria-label={`응답시간 초 입력 필드 ${index + 1}`}
                                        />
                                        <label className="text-sm text-slate-400">초</label>
                                    </div>
                                    {/* Minutes input */}
                                    <div className="flex-1 flex items-center gap-1.5">
                                        <input
                                            type="text"
                                            value={responseTimeData.type === 'minutes' ? responseTimeData.value : ''}
                                            onChange={(e) => handleResponseTimeChange('minutes', e.target.value)}
                                            onBlur={() => onEntryValueBlur?.(entry.id, 'primary')}
                                            disabled={responseTimeData.value !== '' && responseTimeData.type !== 'minutes'}
                                            className={`${baseInputClass} text-slate-200 text-right disabled:bg-slate-800 disabled:text-slate-500`}
                                            aria-label={`응답시간 분 입력 필드 ${index + 1}`}
                                        />
                                        <label className="text-sm text-slate-400">분</label>
                                    </div>
                                    {/* Length input */}
                                    <div className="flex-1 flex items-center gap-1.5">
                                        <input
                                            type="text"
                                            value={responseTimeData.type === 'length' ? responseTimeData.value : ''}
                                            onChange={(e) => handleResponseTimeChange('length', e.target.value)}
                                            onBlur={() => onEntryValueBlur?.(entry.id, 'primary')}
                                            disabled={responseTimeData.value !== '' && responseTimeData.type !== 'length'}
                                            className={`${baseInputClass} text-slate-200 text-right disabled:bg-slate-800 disabled:text-slate-500`}
                                            aria-label={`응답시간 길이 입력 필드 ${index + 1}`}
                                        />
                                        <label className="text-sm text-slate-400">길이(m)</label>
                                    </div>
                                </div>
                            </td>
                        ) : (
                        ...
                        {!isManualEntryMode ? (
                           ...
                           <td className={`px-2 whitespace-nowrap text-sm text-center ${isSequenceRow ? 'py-1 text-red-400 font-semibold' : 'py-2.5 text-slate-300'}`}>
                             {entry.identifier}
                           </td>
                        )}
                    </tr>
