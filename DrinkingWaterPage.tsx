...
const DrinkingWaterPage: React.FC<DrinkingWaterPageProps> = ({
  ...
  // 🔹 P4와 동일한 방식: 오프스크린에서 Snapshot 렌더 후 캡처 → datatable.png 생성
  const captureDatatablePng = useCallback(async (job: DrinkingWaterJob, siteLocationValue: string) => {
    ...
    const base = `${job.receiptNumber}_먹는물_${sanitizeFilenameComponent(job.selectedItem.replace('/', '_'))}`;
  ...
  const handleInitiateSendToKtl = useCallback(async () => {
    ...
    // ✅ Preflight에 datatable.png 파일명도 보여주기 위해 미리 캡처 (파일 자체는 버림)
  ...
  const handleSendToClaydoxConfirmed = useCallback(async () => {
    ...
    try {
      // ✅ 1) datatable.png (P4 체크리스트처럼, 가장 먼저 첨부)
        ...
        <div className="space-y-4 pt-4 border-t border-slate-700">
          ...
          <div className="data-table-container space-y-2">
            ...
            <OcrResultDisplay
              ...
              hasImage={true} // P3는 항상 테이블 표시
              isManualEntryMode={true}
              selectedItem={activeJob.selectedItem}
              onEntryIdentifierChange={() => {}} // P3 미사용
              onEntryIdentifierTPChange={() => {}} // P3 미사용
              onEntryTimeChange={(id, val) => handleEntryValueChange(id, 'primary', val)} // 시간도 값처럼 취급
