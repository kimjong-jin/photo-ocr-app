...
<div className="w-full max-w-3xl bg-slate-900/60 backdrop-blur-sm border border-slate-700/50 rounded-xl shadow-lg p-5 sm:p-7 space-y-5">
  ...
  <h2 className="text-sm font-semibold text-sky-400/70 tracking-widest uppercase mb-1 !mt-0">
    먹는물 분석 (P4)
  </h2>
    ...
    <div className="space-y-2">
        ...
        {(onSaveAllDrafts || onLoadAllDrafts) && (
          <div className="grid grid-cols-4 gap-1">
            <button onClick={() => onSaveDraft?.(jobs.find(j => j.id === activeJobId)?.receiptNumber)} disabled={isSavingDraft || isLoadingDraft} className="py-1 text-[11px] whitespace-nowrap font-medium border border-slate-600 text-slate-300 hover:border-slate-400 hover:text-white rounded transition-colors disabled:opacity-40">
              {isSavingDraft ? '저장 중' : '임시저장'}
            </button>
            <button onClick={() => onLoadDraft?.(jobs.find(j => j.id === activeJobId)?.receiptNumber)} disabled={isSavingDraft || isLoadingDraft} className="py-1 text-[11px] whitespace-nowrap font-medium border border-slate-600 text-slate-300 hover:border-sky-500 hover:text-sky-300 rounded transition-colors disabled:opacity-40">
              {isLoadingDraft ? '로딩 중' : '불러오기'}
            </button>
            <button onClick={() => onSaveAllDrafts?.()} disabled={isSavingDraft || isLoadingDraft} className="py-1 text-[11px] whitespace-nowrap font-medium border border-slate-600 text-slate-300 hover:border-emerald-500 hover:text-emerald-300 rounded transition-colors disabled:opacity-40">
              전체저장
            </button>
            <button onClick={() => onLoadAllDrafts?.()} disabled={isSavingDraft || isLoadingDraft} className="py-1 text-[11px] whitespace-nowrap font-medium border border-slate-600 text-slate-300 hover:border-indigo-400 hover:text-indigo-300 rounded transition-colors disabled:opacity-40">
              전체불러오기
            </button>
          </div>
        )}
