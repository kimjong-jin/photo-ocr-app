...
export interface ExtractedEntry {
  ...
  valueTP?: string;
  identifier?: string;
  isRuleMatched?: boolean; // Keep for type compatibility
}
...
const DrinkingWaterPage: React.FC<DrinkingWaterPageProps> = ({
  ...
  const handleInitiateSendToKtl = useCallback(() => {
    ...
    const hasValues = activeJob.processedOcrData?.some(entry =>
      (entry.value?.trim() || '') !== '' || ((entry.valueTP || '').trim() !== '')
    );
    if (!hasValues) {
  ...
  // ✅ 안전하게 값 존재 여부 확인 (undefined.trim() 방지)
  const isClaydoxDisabled =
    !activeJob ||
    isControlsDisabled ||
    !siteLocation.trim() ||
    !activeJob.processedOcrData?.some(e => (e.value?.trim() || '') !== '' || ((e.valueTP || '').trim() !== ''));
