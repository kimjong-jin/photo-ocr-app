...
const App: React.FC = () => {
  ...
  useEffect(() => {
    // This calculation uses entry.value, which correctly refers to the TN value in TN/TP mode.
    ...
    // Identifiers for range analysis are based on the primary/TN identifier stream
    ...
    try {
      ...
      processedOcrData.forEach(entry => {
        // getConcentrationCategory uses entry.value (TN value in TN/TP mode)
        const category = getConcentrationCategory(entry.value, calculatedBoundaries);
        // getNumericValueFromString uses entry.value (TN value in TN/TP mode)
...
컨텍스트:`;
    ...
    if (item === "TN/TP") {
      ...
      prompt += `\n  "value_tn" (TN 값) 과 "value_tp" (TP 값) 필드를 사용하세요.`;
      prompt += `\n  각 값 필드에는 숫자 값과 함께 텍스트 주석(예: "저", "고 S")이 있다면 그것까지 포함하여 전체 문자열을 추출해주세요. 예: "5.388 저" 또는 "0.123 고 S".`;
      prompt += `\n\nJSON 출력 형식 예시 (항목: TN/TP):
[
  { "time": "2025/04/23 05:00", "value_tn": "46.2 mg/L", "value_tp": "1.2 mg/L" },
  { "time": "2025/04/23 06:00", "value_tn": "5.388 저", "value_tp": "0.1 고 S" },
  { "time": "2025/04/23 07:00", "value_tn": "4.0" } 
]`;
    } else {
      prompt += `\n- 항목/파라미터: ${item}. 이것이 주요 관심 데이터이지만, 다른 시간/값 쌍도 존재한다면 함께 추출해주세요.`;
      prompt += `\n  "Value" 필드에는 숫자 값과 함께 텍스트 주석(예: "저", "고 S")이 있다면 그것까지 포함하여 전체 문자열을 추출해주세요. 예: "5.388 저 그러나 고 S".`;
      prompt += `\n\nJSON 출력 형식 예시 (항목: ${item}):
[
  { "time": "2025/04/23 05:00", "value": "46.2 mg/L" },
  { "time": "2025/04/23 06:00", "value": "5.388 저 그러나 고 S" }
]`;
    }
...
1.  전체 응답은 단일의 유효한 JSON 배열이어야 합니다. ${fenceJson} 와 같은 JSON이 아닌 텍스트, 소개 문구, 설명 또는 마크다운 구분 기호를 포함하지 마세요.
2.  지정된 "항목/파라미터" 관련 데이터를 우선적으로 추출하되, 장치 화면에서 식별 가능한 모든 "Time"(시각) 및 관련 값 쌍을 반드시 추출해야 합니다.
3.  "Time"(시각): 각 시간 항목에 대해, 해당 시간 값 바로 옆이나 매우 근접한 위치에 명확하게 연관된 날짜가 표시된 경우에만 날짜를 포함하세요. 화면 어딘가에 별도로 표시된 날짜를 시간 값에 임의로 연결하지 마세요. 시간만 명확히 표시된 경우, 날짜 없이 시간만 (예: "HH:MM" 또는 "HH:MM:SS") 추출하세요. 날짜와 시간이 명확히 함께 표시된 경우, 표준 형식(예: "YYYY/MM/DD HH:MM" 또는 "YYYY-MM-DD HH:MM:SS")으로 결합하세요.
4.  값 필드 ("value", "value_tn", "value_tp"): 숫자 판독값, 해당 단위(예: "mg/L", "ABS", "°C"), 그리고 관련된 모든 텍스트 주석(예: "저", "고 S", "지정 안함")을 이 단일 값 필드에 결합하세요. 명확하게 연관된 단위나 주석이 없는 경우 숫자만 사용하세요.
5.  항목이 "TN/TP"인 경우:
...
6.  카메라에서 생성된 타임스탬프 및 UI 버튼 텍스트는 실제 데이터의 일부가 아닌 한 제외하세요.
7.  장치 화면에서 "Time" 및 관련 값 쌍을 전혀 찾을 수 없거나 이미지가 인식 가능한 데이터 표시를 포함하지 않는 경우 빈 JSON 배열([])을 반환하세요.
8.  "reactors_input" 또는 "reactors_output" 또는 유사한 마커를 응답에 포함하지 마세요. JSON 응답은 순수하게 데이터 객체의 배열이어야 합니다.
`;
    return prompt;
  };
  ...
  const updateAggregatedDisplayFromProcessedData = (updatedData: ExtractedEntry[] | null) => {
    if (updatedData) {
  ...
  const handleEntryIdentifierChange = useCallback((entryId: string, newIdentifierValue: string | undefined) => {
    setProcessedOcrData(prevData => {
      ...
      setKtlApiCallStatus('idle');
      // updateAggregatedDisplayFromProcessedData(updatedData); // Not needed as this state isn't directly bound to identifier change in terms of raw JSON view
      return updatedData;
    });
  }, []);
  ...
  const handleEntryIdentifierTPChange = useCallback((entryId: string, newIdentifierValue: string | undefined) => {
    setProcessedOcrData(prevData => {
      ...
      setKtlApiCallStatus('idle');
      // updateAggregatedDisplayFromProcessedData(updatedData); // Not needed for raw JSON view
      return updatedData;
    });
  }, []);
  ...
  }, [selectedItem]); // Added selectedItem dependency
  ...
  const handleReorderEntry = useCallback((entryId: string, direction: 'up' | 'down') => {
    setProcessedOcrData(prevData => {
      if (!prevData) return null;
      const entryIndex = prevData.findIndex(entry => entry.id === entryId);
      if (entryIndex === -1) return prevData;

      const newData = [...prevData];
      const entryToMove = newData[entryIndex];

      if (direction === 'up' && entryIndex > 0) {
        newData.splice(entryIndex, 1);
        newData.splice(entryIndex - 1, 0, entryToMove);
      } else if (direction === 'down' && entryIndex < newData.length - 1) {
        newData.splice(entryIndex, 1);
        newData.splice(entryIndex + 1, 0, entryToMove);
      } else {
        return prevData;
  ...
  const handleAutoAssignIdentifiers = useCallback(() => {
    ...
    // For TN/TP mode, boundaries and categories are based on entry.value (TN value).
    ...
    // This assigns to the primary identifier stream (entry.identifier)
    ...
    // getCat uses entry.value (TN value for TN/TP mode)
    ...
    // Priority 1: Z5, S5, Z6, S6, Z7, S7 (LHLHLH - Reproducibility Full Sequence)
      for (let i = 0; i <= n - 6; i++) {
        if (!consumedByIndexCurrentPass[i] && getCat(i) === 'low' &&
            ...
            // getNumericValueFromString uses entry.value (TN value for TN/TP mode)
    ...
    // Priority 2: Z1, Z2, S1, S2 (Initial LLHH - Initial Drift)
      for (let i = 0; i <= n - 4; i++) {
        if (!consumedByIndexCurrentPass[i] && getCat(i) === 'low' &&
    ...
    // Priority 3: M1, M2, M3 (MMM - Accuracy)
      for (let i = n - 3; i >= 0; i--) { 
    ...
    // Priority 4: Z3, Z4, S3, S4 (LLHH - Late Drift)
      for (let i = 0; i <= n - 4; i++) {
        if (!consumedByIndexCurrentPass[i] && getCat(i) === 'low' &&
            !consumedByIndexCurrentPass[i+1] && getCat(i+1) === 'low' &&
            !consumedByIndexCurrentPass[i+2] && getCat(i+2) === 'high' &&
    ...
    // Priority 5: Z5, S5 (LH - Reproducibility Pair Fallback)
    ...
    // Priority 6: Receipt Number Based Fallback for remaining undefined slots (for primary/TN identifiers)
    ...
    for (let dataSlotIndex = 0; dataSlotIndex < n; dataSlotIndex++) {
        if (newAssignments[dataSlotIndex] === undefined && !consumedByIndexCurrentPass[dataSlotIndex]) {
            ...
            while(tempPatternSearchIndex < patternBasedIdentifiers.length && !filledThisDataSlot) {
                ...
                // Check if suggestedId is valid for TN stream if in TN/TP mode
    ...
    setProcessedOcrData(prevData => {
      ...
      const updatedData = prevData.map((entry, idx) => ({
        ...entry,
        identifier: newAssignments[idx], // This is the TN/primary identifier
        isRuleMatched: newRuleMatchedFlags[idx]
        // identifierTP remains unchanged by this auto-assignment logic
      }));
      updateAggregatedDisplayFromProcessedData(updatedData); // update with potentially TN/TP data
      setKtlApiCallStatus('idle');
      return updatedData;
    });
  ...
  const handleDownloadStampedImages = useCallback(async () => {
    ...
    try {
      ...
      const sanitizedItem = sanitizeFilenameComponent(selectedItem === "TN/TP" ? "TN_TP" : selectedItem); // Adjust filename for TN/TP
      ...
      for (let i = 0; i < selectedImages.length; i++) {
        ...
        const stampedDataUrl = await generateStampedImage(
          ...
          inspectionStartDate, selectedItem // Pass selectedItem to stamping
        );
        const originalFileName = imageInfo.file.name;
        const extension = originalFileName.substring(originalFileName.lastIndexOf('.') + 1) || 'png';
        const newFileName = `${receiptNumber}_${sanitizedSiteLocation}_${sanitizedItem}_${i + 1}.${extension}`;
      ...
      const zipFileNameBase = `${receiptNumber.replace(/-/g, '_')}_${sanitizedSiteLocation}_${sanitizedItem}`;
      const zipFileName = `${zipFileNameBase}_images.zip`;
  ...
  const handleSendToClaydox = useCallback(async () => {
    ...
    // Check for identifiers based on selectedItem
    ...
    // uniqueIdentifiersForNaming should be based on the primary (TN) identifier stream
    // as image naming is generally tied to one primary identifier per image.
    const allUniquePrimaryIdentifiersFromOCR = Array.from(new Set(processedOcrData.filter(e => e.identifier).map(e => e.identifier!)));
    const identifiersActuallyUsedForFileNaming: string[] = allUniquePrimaryIdentifiersFromOCR.slice(0, selectedImages.length);

    const payload: ClaydoxPayload = {
      receiptNumber,
      siteLocation,
      item: selectedItem, // Pass current selectedItem
      inspectionStartDate: inspectionStartDate || undefined,
      ocrData: processedOcrData, // Send the full data, service will pick relevant fields
      updateUser: userName,
      uniqueIdentifiersForNaming: identifiersActuallyUsedForFileNaming, 
    };
    ...
    try {
      ...
      for (let i = 0; i < selectedImages.length; i++) {
        ...
        const stampedDataUrl = await generateStampedImage(
          ...
          siteLocation,
          inspectionStartDate,
          selectedItem // Pass selectedItem for stamping text
        );
        const stampedBlob = dataURLtoBlob(stampedDataUrl);
        const originalFileName = imageInfo.file.name;
        const extension = originalFileName.substring(originalFileName.lastIndexOf('.') + 1) || 'png';
        // Filename uses the sanitized selectedItem (or TN_TP)
        const actualKtlFileName = `${receiptNumber}_${sanitizedSite}_${sanitizedItemName}_${i + 1}.${extension}`;
        
        console.log(`[App.tsx] Preparing file for KTL: ${actualKtlFileName}`);
        const newFile = new File([stampedBlob], actualKtlFileName, { type: imageInfo.mimeType });
        filesToUpload.push(newFile);
      }

      // Pass selectedItem to sendToClaydoxApi so it knows how to structure the KTL JSON
      const response = await sendToClaydoxApi(payload, filesToUpload, selectedItem); 
  ...
  const claydoxDisabledCondition = () => {
    if (!processedOcrData || !areContextFieldsValid || isLoading || isDownloadingStamped || isSendingToClaydox || !userName) return true;
    const hasIdentifiers = processedOcrData.some(entry => 
        selectedItem === "TN/TP" ? (entry.identifier || entry.identifierTP) : entry.identifier
    );
  ...
  // Auto-assign is based on primary (TN) values/identifiers
  ...
  const ktlJsonPreview = useMemo(() => {
    ...
    const payload: ClaydoxPayload = {
      ...
      item: selectedItem,
      inspectionStartDate: inspectionStartDate || undefined,
      ocrData: processedOcrData, // Send full data
      updateUser: userName,
      uniqueIdentifiersForNaming: identifiersActuallyUsedForFileNaming,
    };
    return generateKtlJsonForPreview(payload, selectedItem); // Pass selectedItem
  }, [receiptNumber, siteLocation, selectedItem, inspectionStartDate, processedOcrData, areContextFieldsValid, userName, selectedImages.length]);
      ...
      <main className="w-full max-w-3xl bg-slate-800 shadow-2xl rounded-xl p-6 sm:p-8 space-y-6">
        <AnalysisContextForm
          ...
          onSelectedItemChange={(val) => { 
            setSelectedItem(val); 
            // If item changes, existing OCR data might not be directly applicable in the new context for display.
            // Consider if processedOcrData should be reset or transformed here.
            // For now, just change item and KTL status.
            setKtlApiCallStatus('idle'); 
          }}
        ...
        <OcrResultDisplay
          ...
          onEntryIdentifierChange={handleEntryIdentifierChange} // For primary/TN identifier
          onEntryIdentifierTPChange={handleEntryIdentifierTPChange} // For TP identifier
          onEntryTimeChange={handleEntryTimeChange}
          onEntryPrimaryValueChange={handleEntryPrimaryValueChange} // For primary/TN value
          onEntryValueTPChange={handleEntryValueTPChange} // For TP value
          onReorderEntry={handleReorderEntry}
          availableIdentifiers={IDENTIFIER_OPTIONS} // All possible identifiers
