...
import KtlPreflightModal, { KtlPreflightData } from './components/KtlPreflightModal'; // 추가
...
const App: React.FC = () => {
  ...
  const hypotheticalKtlFileNamesForPreview = useMemo(() => {
    ...
    return selectedImages.map((imageInfo, i) => {
        const extension = getFileExtension(imageInfo.file.name, imageInfo.mimeType);
        return `${receiptNumber}_${sanitizedSite}_${sanitizedItemName}_${i + 1}.${extension}`;
    });
  }, [selectedImages, receiptNumber, siteLocation, selectedItem]);
  ...
  const ktlJsonPreview = useMemo(() => {
    ...
    const hasIdentifiers = processedOcrData.some(entry => 
     ...
     // Allow preview even if no identifiers, but send button will be disabled.
    // if (!hasIdentifiers && processedOcrData.length > 0) return null; 
  ...
  }, [receiptNumber, siteLocation, selectedItem, inspectionStartDate, processedOcrData, areContextFieldsValid, userName, selectedImages.length, hypotheticalKtlFileNamesForPreview]);
...
`;
  ...
  const handleDownloadStampedImages = useCallback(async () => {
    ...
    try {
      ...
      for (let i = 0; i < selectedImages.length; i++) {
        ...
        // Use raw Hangul filename for download
      ...
      for (const stampedImage of stampedImagesData) {
        ...
        zip.file(stampedImage.name, blob); // Use raw Hangul name here
      }
      
      // Use raw Hangul filename for ZIP
      ...
      link.download = zipFileName; // Use raw Hangul name here
  ...
  const handleInitiateSendToKtl = useCallback(() => {
    ...
    // ktlJsonPreview and hypotheticalKtlFileNamesForPreview are now declared before this callback
    if (!ktlJsonPreview) { 
    ...
    setKtlPreflightData({
        jsonPayload: ktlJsonPreview, // Safe to use
        fileNames: hypotheticalKtlFileNamesForPreview, // Safe to use
        context: {
            receiptNumber,
            siteLocation,
            selectedItem,
    ...
    userName, 
    ktlJsonPreview, // Now a valid dependency as it's declared earlier
    hypotheticalKtlFileNamesForPreview, // Now a valid dependency
    receiptNumber,
    siteLocation,
    selectedItem,
    inspectionStartDate
  ...
  const handleSendToClaydoxConfirmed = useCallback(async () => {
    setKtlPreflightModalOpen(false); // Close modal first
    if (!processedOcrData || !areContextFieldsValid || !userName ) {
      // This check is mostly redundant due to `handleInitiateSendToKtl` but good for safety
    ...
    try {
      ...
      const actualKtlFileNames: string[] = []; // These will be RAW Hangul names
      const sanitizedSite = sanitizeFilenameComponent(siteLocation); 
      const sanitizedItemName = sanitizeFilenameComponent(selectedItem === "TN/TP" ? "TN_TP" : selectedItem); 
      ...
      for (let i = 0; i < selectedImages.length; i++) {
        ...
        const rawKtlFileName = `${receiptNumber}_${sanitizedSite}_${sanitizedItemName}_${i + 1}.${extension}`;
        actualKtlFileNames.push(rawKtlFileName);
        
        console.log(`[App.tsx] Preparing file for KTL. Raw Hangul filename: ${rawKtlFileName}`);
        const newFile = new File([stampedBlob], rawKtlFileName, { type: imageInfo.mimeType }); 
  ...
  const claydoxDisabledCondition = () => {
    ...
    if (!hasIdentifiers && processedOcrData.length > 0) return true; // Prevent send if data exists but no identifiers
    if (processedOcrData.length === 0 && selectedImages.length > 0) return true; // Prevent send if images exist but no OCR data (e.g. after clear OCR then trying to send)
    if (processedOcrData.length === 0 && selectedImages.length === 0) return true; // Nothing to send
    return false;
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 to-slate-800 text-slate-100 flex flex-col items-center p-4 sm:p-8 font-[Inter]">
      ...
      <main className="w-full max-w-3xl bg-slate-800 shadow-2xl rounded-xl p-6 sm:p-8 space-y-6">
        ...
        <OcrControls
          ...
          onInitiateSendToKtl={handleInitiateSendToKtl} // 변경됨
