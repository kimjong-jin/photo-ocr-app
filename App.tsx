...
4.  값 필드 ("value", "value_tn", "value_tp"): **오직 숫자 부분만** 추출해주세요. 이미지에 "N 89.629 mgN/L [M_]"와 같이 표시되어 있다면, 해당 값 필드에는 "89.629"와 같이 순수한 숫자 문자열만 포함해야 합니다. 접두사(예: "N "), 단위(예: "mgN/L"), 텍스트 주석(예: "[M_]", "(A)", "저", "고 S") 등은 **모두 제외**해야 합니다. 만약 숫자 값을 명확히 식별할 수 없다면, 해당 값 필드를 JSON 객체에서 생략하거나 빈 문자열 ""로 설정해주세요.
5.  항목이 "TN/TP"인 경우:
      ...
      <main className="w-full max-w-3xl bg-slate-800 shadow-2xl rounded-xl p-6 sm:p-8 space-y-6">
        <AnalysisContextForm
          ...
          onSelectedItemChange={(val) => { 
            ...
            setProcessedOcrData(null); // Reset OCR data when item changes for clarity
            setAggregatedOcrTextForDisplay(null);
            setRangeDifferenceResults(null);
            setConcentrationBoundaries(null);
            setKtlApiCallStatus('idle'); 
