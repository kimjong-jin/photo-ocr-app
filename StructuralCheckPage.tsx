...
import {
  ...
  StructuralCheckSubItemData // Ensure this is imported
...
import { sendBatchStructuralChecksToKtlApi, StructuralCheckPayloadForKtl, generateStructuralKtlJsonForPreview, ClaydoxJobPhoto, generateCompositeImageNameForKtl, generateZipFileNameForKtl } from './services/claydoxApiService'; // Added generateZipFileNameForKtl
...
// --- Interfaces ---
// StructuralCheckSubItemData is now imported from shared/structuralChecklists.ts
...
// State to track if a specific analysis type has been performed for a photo in a job
// {[jobId: string]: {[photoIndex: number]: Set<AnalysisType>}}
...
const sanitizeFilenameComponent = (component: string): string => {
  if (!component) return '';
  // Allow hyphens, remove other problematic characters for filenames
...
const StructuralCheckPage: React.FC<StructuralCheckPageProps> = ({ userName }) => {
  ...
  const [isLoading, setIsLoading] = useState<boolean>(false); // Used for KTL submission process
  ...
  const [isAnalyzingDetail, setIsAnalyzingDetail] = useState<boolean>(false); // For single item AI analysis
  ...
  const handleActiveJobPhotosSet = useCallback((images: ImageInfo[]) => {
    ...
    setAnalysisStatusForPhotos(prevStatus => {
        ...
        if (activeJobId) {
            delete newStatus[activeJobId]; // Clear analysis status for this job as photos changed
        }
        return newStatus;
    });
  ...
  const handleNextActiveJobImage = useCallback(() => {
    ...
    setQuickAnalysisFeedback(null); // Clear feedback when changing photo
    ...
    setJobs(prevJobs =>
      prevJobs.map(job => {
        if (job.id === jobId) {
          ...
          if (itemName === "측정범위확인" || itemName === "측정방법확인" || itemName === "표시사항확인" || itemName === "운용프로그램확인" || itemName === "정도검사 증명서") {
            // Do not clear quickAnalysisFeedback here as it's set by the AI analysis itself.
          }
  ...
  const handleAnalyzeChecklistItemDetail = useCallback(async (
    ...
    const photoToProcess = photosToAnalyze[0]; // For quick analysis, always use the currently selected photo.
...
JSON 문자열 외부에는 어떠한 추가 설명이나 텍스트도 포함하지 마세요.`;
        modelConfigForCall = { responseMimeType: "application/json" }; // Ensure this is set
      } else if (itemNameForAnalysis === "측정범위확인") {
...
- Do NOT include any labels (like "Program No.:") in your response, only the 12-digit version value itself or the "판별 불가" message.`;
  ...
  const generateChecklistImageForJob = async (job: StructuralJob): Promise<ImageInfo> => {
    ...
    const ChecklistCaptureComponent = (
      <div style={{ padding: '20px', backgroundColor: '#1f2937', color: 'white', border: '1px solid #374151' }}>
        ...
        {CHECKLIST_DEFINITIONS[job.mainItemKey].map((itemName, index) => {
          ...
          if (itemName === "정도검사 증명서") {
            ...
            try {
             ...
             noteToDisplay = `${prefix}: ${noteToDisplay}`;
          }
          ...
          <div key={itemName} style={{ padding: '0.5rem 0.25rem', borderBottom: '1px solid #374151', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', minHeight: '40px' }}>
            <div style={{ flex: 1, marginRight: '1rem' }}>
              <div>
                <span style={{ marginRight: '0.5rem', color: '#cbd5e1' }}>{index + 1}.</span>
                <span style={{ color: '#e5e7eb' }}>{itemName}</span>
              </div>
              {itemData?.confirmedAt && (
               ...
               {noteToDisplay && (
                 <span style={{ fontSize: '0.8rem', color: '#a5f3fc', marginLeft: 'calc(0.5rem + 0.7ch)', display: 'block', marginTop: '0.125rem', fontWeight: 500, whiteSpace: 'pre-wrap', wordBreak: 'break-all' }}>
                    {noteToDisplay}
                  </span>
               )}
               {specialNoteToDisplay && (
  ...
  const hypotheticalFileNamesForPreflight = useMemo(() => {
    ...
    return Array.from(new Set(filesArray)).sort(); // Ensure unique and sorted
  }, [jobs, siteLocation]);
      ...
      {jobs.length > 0 && (
        <div className="mt-6 pt-4 border-t border-slate-700">
          <ActionButton
            ...
            disabled={!isBatchValidForSubmission || isLoading || isAnalyzingDetail}
            fullWidth
          >
