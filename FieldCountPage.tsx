...
const FieldCountPage: React.FC<FieldCountPageProps> = ({ userName, jobs, setJobs, activeJobId, setActiveJobId, siteLocation, onDeleteJob }) => {
  ...
  const handleImagesSet = useCallback((newlySelectedImages: ImageInfo[]) => {
    ...
    const photosWithUids: JobPhoto[] = newlySelectedImages.map(img => ({ ...img, uid: self.crypto.randomUUID() }));
    updateActiveJob(job => {
  ...
  const handleCameraCapture = useCallback((file: File, base64: string, mimeType: string) => {
    const photoWithUid: JobPhoto = { file, base64, mimeType, uid: self.crypto.randomUUID() };
    updateActiveJob(job => {
        const newPhotos = [...(job.photos || []), photoWithUid];
  ...
  const handleSendToClaydoxConfirmed = useCallback(async () => {
    ...
    try {
        ...
        const response = await sendToClaydoxApi(payload, [compositeFile, zipFile], activeJob.selectedItem, [`${baseName}_composite.jpg`, `${baseName}_Compression.zip`]);
    ...
    for (let i = 0; i < jobsToSend.length; i++) {
        ...
        try {
            ...
            const baseName = `${job.receiptNumber}_${sanitizeFilename(siteLocation)}_${sanitizeFilename(job.selectedItem.replace('/', '_'))}`;
