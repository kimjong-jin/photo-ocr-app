...
const FieldCountPage: React.FC<FieldCountPageProps> = ({ userName }) => {
  ...
  const ktlJsonPreview = useMemo(() => {
    ...
    const allUniquePrimaryIdentifiersFromOCR = Array.from(new Set(processedOcrData.flatMap(e => [e.identifier, e.identifierTP]).filter(Boolean) as string[]));
  ...
  }, [receiptNumber, siteLocation, selectedItem, processedOcrData, areContextFieldsValid, userName, hypotheticalKtlFileNamesForPreview]);
  ...
  const handleExtractText = useCallback(async () => {
    if (selectedImages.length === 0 || !areContextFieldsValid) {
      setProcessingError(selectedImages.length === 0 ? "먼저 이미지를 선택해주세요." : "모든 필수 항목(접수번호, 현장, 항목)을 입력해주세요.");
      return;
    }
    setIsLoading(true);
    setProcessingError(null);
  ...
  const handleInitiateSendToKtl = useCallback(() => {
    if (userName === "게스트" || !processedOcrData || processedOcrData.length === 0 || selectedImages.length === 0 || !areContextFieldsValid || !ktlJsonPreview) {
  ...
  }, [processedOcrData, selectedImages, areContextFieldsValid, userName, ktlJsonPreview, hypotheticalKtlFileNamesForPreview, receiptNumber, siteLocation, selectedItem]);
  ...
  const handleSendToClaydoxConfirmed = useCallback(async () => {
    ...
    try {
        const payload: ClaydoxPayload = {
            receiptNumber, siteLocation, item: selectedItem, updateUser: userName,
            ocrData: processedOcrData,
            uniqueIdentifiersForNaming: Array.from(new Set(processedOcrData.flatMap(e => [e.identifier, e.identifierTP]).filter(Boolean) as string[])),
        ...
        const compositeDataUrl = await generateCompositeImage(imageInfosForComposite, { receiptNumber, siteLocation, item: selectedItem }, 'image/jpeg');
        ...
        for (let i = 0; i < selectedImages.length; i++) {
            ...
            const stampedDataUrl = await generateStampedImage(imageInfo.base64, imageInfo.mimeType, receiptNumber, siteLocation, '', selectedItem);
  ...
  }, [processedOcrData, receiptNumber, siteLocation, selectedItem, areContextFieldsValid, selectedImages, userName]);
