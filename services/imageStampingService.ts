...
export const generateStampedImage = (
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      const fontSize = Math.max(16, Math.min(img.width / 35, img.height / 25)); // Adjusted for potentially smaller individual images
      ...
      if (inspectionDate && inspectionDate.trim() !== '') textLines.push(`검사시작일: ${inspectionDate}`); // This line will only add if inspectionDate is non-empty
      ...
      textLines.forEach((line, index) => {
        // Adjust Y for each line to be within the drawn rectangle
...
export const generateCompositeImage = (
  images: Base64ImageSource[], // Changed type
