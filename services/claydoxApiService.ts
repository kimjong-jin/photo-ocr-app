  ...
  payload.ocrData.forEach(entry => {
    ...
    if (entry.identifier && typeof entry.value === 'string') {
      ...
      if (valueToUse !== null) {
        ...
        if (compositePhotoFileName && ALLOWED_PHOTO_LINK_UIDS_PRIMARY.has(entry.identifier)) {
          labviewItems[`${entry.identifier}_사진`] = compositePhotoFileName;
        }
        // Add _압축 if the primary/TN identifier is "M1"
        if (zipPhotoFileName && entry.identifier === "M1") {
          labviewItems[`M1_압축`] = zipPhotoFileName; // Key is explicitly "M1_압축"
        }
      }
    }
    ...
    if (selectedItem === "TN/TP" && entry.identifierTP && typeof entry.valueTP === 'string') {
      ...
      if (valueToUseTP !== null) {
        ...
        if (compositePhotoFileName && ALLOWED_PHOTO_LINK_UIDS_TP.has(entry.identifierTP)) {
          // Ensure we don't overwrite an _사진 field if the identifier happened to be reused (e.g. S3)
          // and was already processed as a primary identifier. This is less likely if TN and TP identifiers are distinct.
          if (!labviewItems[`${entry.identifierTP}_사진`]) {
             labviewItems[`${entry.identifierTP}_사진`] = compositePhotoFileName;
          }
        }
        // No _압축 logic here for identifierTP as "M1" is not in TP_IDENTIFIERS from shared/constants.
        // The request is for "M1_압축", implying it's tied to the "M1" identifier,
        // which is typically handled by the `entry.identifier` (primary/TN) part above.
      }
    }
  });
