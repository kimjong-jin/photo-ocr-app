...
import {
  ...
  generateA4LandscapeReportPages,
  dataURLtoBlob,
  ...
  type A4Base64Image,
  type ReportMeta,
  generateExtraPhotoA4Pages,
  type ExtraPhotoForA4,
...
// ───────────────────────────────────────────────────────────────
// 📦 무결성 검증 유틸리티 (SHA-256 + integrity.json)
// ───────────────────────────────────────────────────────────────

/** Blob → SHA-256 16진수 문자열 (Web Crypto API, 브라우저 전용) */
...
/** "YYYY-MM-DD" 또는 "YYYY/MM/DD" 문자열 → Date (UTC 자정) */
function parseInspectionDate(dateStr?: string): Date {
  if (!dateStr) return new Date();
  const iso = dateStr.replace(/\//g, '-').split('T')[0]; // YYYY-MM-DD
...
/**
 * integrity.json 내용 생성
 * 각 Blob을 SHA-256 해싱하여 파일명·해시·크기를 기록.
 * ZIP에 포함하면 수신자가 파일 변조 여부를 검증할 수 있음.
 */
async function buildIntegrityManifest(
  entries: { name: string; blob: Blob }[],
  meta: { receiptNumber: string; siteLocation: string; item: string; inspectionEndDate?: string }
): Promise<string> {
  const fileEntries: IntegrityEntry[] = await Promise.all(
  ...
  const manifest: IntegrityManifest = {
    schema: 'claydox-integrity-v1',
    generator: 'claydox.ktl.re.kr',
    generatedAt: new Date().toISOString(),
    receiptNumber: meta.receiptNumber,
    siteLocation: meta.siteLocation,
    item: meta.item,
    inspectionEndDate: meta.inspectionEndDate || '',
    files: fileEntries,
  };
...
const constructMergedLabviewItemForStructural = (
  ...
  masterCompositeImageNamesOnServer?: string[], // ← 다중 파일명 배열
  ...
  jobsInGroup.forEach((payload) => {
    if (payload.postInspectionDateValue) {
      ...
      mergedItems[periodKey] = ktlPeriodValue || '선택되지 않았습니다';
    } else {
      const periodKeySuffix = payload.mainItemKey === 'TP' ? 'P' : payload.mainItemKey === 'Cl' ? 'C' : '';
      mergedItems[`구조_사후검사일${periodKeySuffix}_주기`] = '선택되지 않았습니다';
    }

    if (payload.mainItemKey === 'TOC') {
      const emissionStandardData = payload.checklistData[EMISSION_STANDARD_ITEM_NAME];
      mergedItems['구조_배출기준_입력값'] =
        (emissionStandardData?.notes?.trim()) || '입력되지 않았습니다';
      const responseTimeData = payload.checklistData[RESPONSE_TIME_ITEM_NAME];
      mergedItems['구조_응답시간_입력값'] =
        (responseTimeData?.notes?.trim()) || '입력되지 않았습니다';
    }
    ...
    Object.entries(payload.checklistData).forEach(([checklistItemName, data]) => {
      ...
      if (checklistItemName !== '기기번호 확인') {
        ...
        if (data.status === '선택 안됨' || !data.status?.trim()) {
          statusForKtl = '선택되지 않았습니다';
      ...
      if (checklistItemName === '측정범위확인') {
        ...
        mergedItems[`${baseKeyForData}_노트`] = effectiveRangeString || '선택되지 않았습니다';
      ...
      } else if (checklistItemName === '정도검사 증명서') {
        if (data.notes) {
          try {
            ...
            switch (certDetails.presence) {
              ...
              default: statusText = (certDetails.presence && certDetails.presence !== 'not_selected')
                ? String(certDetails.presence)
                : '선택되지 않았습니다';
            }
          ...
          mergedItems[`${baseKeyForData}_세부상태`] = '선택되지 않았습니다';
        }
      ...
      } else if (checklistItemName === '측정방법확인') {
        // 측정방법확인: 내용 없으면 "선택되지 않았습니다"
        mergedItems[`${baseKeyForData}_노트`] = data.notes?.trim() || '선택되지 않았습니다';
      } else {
        // 기타 일반 항목
    ...
    if (masterCompositeImageNamesOnServer?.length) {
      masterCompositeImageNamesOnServer.forEach((name, idx) => {
        mergedItems[`${baseFileKey}_개별사진묶음_${idx + 1}`] = name;
      });
    }
...
export const generateStructuralKtlJsonForPreview = (
  ...
  // 미리보기용: 단일 이름을 배열로 래핑
  const hypotheticalNames = hypotheticalCompositeImageName ? [hypotheticalCompositeImageName] : undefined;
...
export const generateCompositeImageNameForKtl = (receiptNumber: string): string => {
  ...
  return `${sanitizedReceipt}_composite_001.jpg`;
};

/** 점수별 합성 파일명 배열 생성 (6장/A4, 다중 페이지) */
export const generateCompositeImageNamesForKtl = (receiptNumber: string, pageCount: number): string[] => {
  const sanitized = sanitizeFilename(receiptNumber);
  return Array.from({ length: pageCount }, (_, i) =>
    `${sanitized}_composite_${String(i + 1).padStart(3, '0')}.jpg`
  );
};
...
export const sendSingleStructuralCheckToKtlApi = async (
  ...
  if (job.photos && job.photos.length > 0) {
    ...
    try {
      ...
      // ✅ 가로 A4 성적서 스타일 (흰 배경 + 헤더 + 캡션)
      const imagesForLandscape: A4Base64Image[] = job.photos.map(p => ({
        base64: p.base64,
        mimeType: p.mimeType,
      ...
      const reportMeta: ReportMeta = {
        ...
        postInspectionDate: job.postInspectionDate || '',
        userName: userNameGlobal,
      };
      const a4Pages = await generateA4LandscapeReportPages(imagesForLandscape, reportMeta, { quality: 0.92 });
      compositeFileNamesOnServer = generateCompositeImageNamesForKtl(job.receiptNumber, a4Pages.length);
      a4Pages.forEach((page, idx) => {
        filesToUpload.push(new File([dataURLtoBlob(page)], compositeFileNamesOnServer[idx], { type: 'image/jpeg' }));
      });
    ...
    // 2b) 스탬프된 개별사진 ZIP 생성 + SHA-256 무결성 검증
    onProgress('(3/4) [P1 구조 확인] 참고사진 ZIP 파일 생성 중...');
    try {
      const zip = new JSZip();
      const endDate = parseInspectionDate(job.inspectionEndDate);
      const itemName = MAIN_STRUCTURAL_ITEMS.find((it) => it.key === job.mainItemKey)?.name || job.mainItemKey;
      const integrityEntries: { name: string; blob: Blob }[] = [];

      // ✅ 원본 사진 그대로 ZIP에 추가 (A4 렌더링/스탬프 없음)
      for (const photo of job.photos) {
        const safeName = safeNameWithExt(photo.file.name, photo.mimeType);
        const photoBlob = dataURLtoBlob(`data:${photo.mimeType};base64,${photo.base64}`);
        zip.file(safeName, photoBlob, { date: endDate });
      }

      zipFileNameOnServer = generateZipFileNameForKtl(job.receiptNumber);
      const endDateTime = endDate.getTime();
      const zipBlob = await zip.generateAsync({ type: 'blob' });
      // File lastModified도 검사종료일로 설정
      filesToUpload.push(new File([zipBlob], zipFileNameOnServer, {
        type: 'application/zip',
        lastModified: endDateTime,
      }));
    } catch (e: any) {
  ...
  const mergedLabviewItem = constructMergedLabviewItemForStructural(
    ...
    siteNameGlobal,
    gpsAddressGlobal,
    compositeFileNamesOnServer.length > 0 ? compositeFileNamesOnServer : undefined,
    zipFileNameOnServer
  );
...
export const sendBatchStructuralChecksToKtlApi = async (
  ...
  for (const receiptNo in jobsByReceiptNumber) {
    ...
    if (photosForThisReceipt.length > 0) {
      ...
      // ✅ 단건 전송과 동일: generateA4LandscapeReportPages 방식으로 통일
      // (스탬프 겹침 없음, 가로 A4 성적서 형식, 헤더 포함)
      const firstJobForReceipt = jobs.find((j) => j.receiptNumber === receiptNo);
      const imagesForLandscape: A4Base64Image[] = photosForThisReceipt.map((p) => {
        const sourceJob = jobs.find((j) => j.id === p.jobId);
        return {
          base64: (p as any).base64 || '',
          mimeType: p.mimeType,
          comment: sourceJob?.photoComments?.[p.uid] || '',
        };
      });
      const reportMeta: ReportMeta = {
        ...
        siteLocation: siteNameGlobal,
        item: itemSummaryForStamp,
        inspectionStartDate: firstJobForReceipt?.inspectionStartDate || '',
        inspectionEndDate: firstJobForReceipt?.inspectionEndDate || '',
        postInspectionDate: firstJobForReceipt?.postInspectionDate || '',
        userName: userNameGlobal,
      };

      try {
        const a4BatchPages = await generateA4LandscapeReportPages(imagesForLandscape, reportMeta, { quality: 0.92 });
        const batchCompositeNames = generateCompositeImageNamesForKtl(receiptNo, a4BatchPages.length);
        a4BatchPages.forEach((page, idx) => {
          filesToUploadDirectly.push(new File([dataURLtoBlob(page)], batchCompositeNames[idx], { type: 'image/jpeg' }));
        });
        receiptToCompositeFileNamesMap.set(receiptNo, batchCompositeNames);
    ...
    if (photosForThisReceipt.length > 0) {
      const zip = new JSZip();
      // ✅ 원본 사진 그대로 ZIP에 추가 (스탬프/가공 없음)
      for (const photo of photosForThisReceipt) {
        try {
          const originalBase64 = photo.base64Original || (photo as any).base64 || '';
          const safeName = safeNameWithExt(photo.file.name, photo.mimeType);
          const photoBlob = dataURLtoBlob(`data:${photo.mimeType};base64,${originalBase64}`);
          const sourceJob = jobs.find((j) => j.id === photo.jobId);
          const sourceEndDate = parseInspectionDate(sourceJob?.inspectionEndDate);
          zip.file(safeName, photoBlob, { date: sourceEndDate });
        } catch (zipError: any) {
          console.error(`[ClaydoxAPI - P1] Error adding photo ${photo.file.name} to ZIP:`, zipError);
        }
      }
  ...
  for (const receiptNo in jobsByReceiptNumber) {
    ...
    const compositeFileNamesForThisReceipt = receiptToCompositeFileNamesMap.get(receiptNo);
...
/**
 * SS/pH/DO 진행순서(sequence_code) 생성
 * aiAnalysisResult 키를 timestamp 순으로 정렬 후 센서 타입별 규칙으로 변환
 */
const EXCLUDED_AI_META_KEYS = new Set([
  'phases', 'responseTimeInSeconds', 'responseStartPoint', 'responseEndPoint',
  'responseError', 'identifiedPatternNumber', 'isReagent', 'st', 'en',
]);

function extractCsvSeqPart(key: string, sensorType: SensorType): string | null {
  // 현장N 항상 제외
  if (/^현장/.test(key)) return null;

  if (sensorType === 'SS') {
    // m1→M, z2→Z, s3→S : 숫자 제거 후 대문자
    const base = key.replace(/\d/g, '').toUpperCase();
    return base.length > 0 ? base : null;
  }

  if (sensorType === 'PH') {
    // (a)_4_1, (a)_7_2, (b)_4_1, (c)_10_1 → 버퍼 번호만 (label.toLowerCase()로 저장되므로 소문자 매칭)
    const bufferMatch = key.match(/\([A-Za-z]+\)_(\d+)_\d+/);
    if (bufferMatch) return bufferMatch[1];
    // 4_10, 7_15, 4_20 등 온도보상형 → 버퍼 번호만 (온도: 10/15/20/25/30)
    const tempCompMatch = key.match(/^(\d+)_(10|15|20|25|30)$/);
    if (tempCompMatch) return tempCompMatch[1];
    return null;
  }

  if (sensorType === 'DO') {
    // 20_S_1, 30_S_2 등 온도보상형 → 앞 숫자만
    const doTempMatch = key.match(/^(\d+)_[A-Za-z]/);
    if (doTempMatch) return doTempMatch[1];
    // (A)_S1, (A)_S2 → 알파벳만
    const parenthesisMatch = key.match(/\([A-Za-z]+\)_([A-Za-z]+)\d*/);
    if (parenthesisMatch) return parenthesisMatch[1].toUpperCase();
    // S_1, Z_1, Z_2 → 알파벳만
    const underscoreMatch = key.match(/^([A-Za-z]+)_\d+$/);
    if (underscoreMatch) return underscoreMatch[1].toUpperCase();
    // S1, Z1 단순형 → 알파벳만
    const simpleMatch = key.match(/^([A-Za-z]+)\d+$/);
...
function generateCsvSequenceCode(job: CsvGraphJob): string {
  if (!job.aiAnalysisResult) return '';
  const sensorType = job.sensorType;
  if (sensorType !== 'SS' && sensorType !== 'PH' && sensorType !== 'DO') return '';

  // timestamp가 있는 포인트만 수집
  const points: { key: string; ts: string }[] = [];
  for (const [key, val] of Object.entries(job.aiAnalysisResult)) {
    if (EXCLUDED_AI_META_KEYS.has(key)) continue;
    if (val && typeof val === 'object' && typeof val.timestamp === 'string') {
      points.push({ key, ts: val.timestamp });
    }
  }
  // timestamp 오름차순 정렬
  ...
  for (const { key } of points) {
    const part = extractCsvSeqPart(key, sensorType);
    if (part !== null) sequence += part;
  }
  return sequence;
}
...
export const sendCsvGraphToKtlApi = async (
  ...
  try {
    ...
    // ✅ SS/pH/DO 진행순서(sequence_code) 추가
    const seqCode = generateCsvSequenceCode(job);
    if (seqCode) {
      labviewItemObject['sequence_code'] = seqCode;
      console.log(`${logIdentifier} sequence_code:`, seqCode);
    }
...
// ─────────────────────────────────────────────────────────────────────────────
// 📎 추가 사진자료 KTL 전송
// Claydox 매칭: /uploadfiles → [A4 JPEG 파일(들) + 원본.zip]
//              /env         → LABVIEW_GUBN=추가사진자료, LABVIEW_RECEIPTNO=접수번호
// ─────────────────────────────────────────────────────────────────────────────

export const sendExtraPhotosToClaydox = async (
  receiptNumber: string,
  ...
  const rSafe = sanitize(receiptNumber);

  onProgress?.('A4 합성본 생성 중...');
  const a4ForGen: ExtraPhotoForA4[] = photos.map(p => ({ file: p.file, photoType: p.photoType, comment: p.comment }));
  const a4DataUrls = await generateExtraPhotoA4Pages(a4ForGen, { receiptNumber, itemName });
  const a4Files: File[] = a4DataUrls.map((dataUrl, i) => {
    const blob = dataURLtoBlob(dataUrl);
    return new File([blob], `${rSafe}_추가사진자료_A4_${i + 1}.jpg`, { type: 'image/jpeg' });
  });

  onProgress?.('원본 ZIP 생성 중...');
  const zip = new JSZip();
  photos.forEach((p, i) => {
    const ext = p.file.type === 'image/png' ? 'png' : 'jpg';
    const label = sanitize(p.comment || String(i + 1)).slice(0, 30);
    zip.file(`${String(i + 1).padStart(2, '0')}_${label}.${ext}`, p.file);
  });
  const zipBlob = await zip.generateAsync({ type: 'blob', compression: 'DEFLATE' });
  const zipFile = new File([zipBlob], `${rSafe}_추가사진자료_원본.zip`, { type: 'application/zip' });

  const filesToUpload: File[] = [...a4Files, zipFile];

  onProgress?.(`파일 업로드 중 (${filesToUpload.length}개)...`);
  const formData = new FormData();
  filesToUpload.forEach(f => formData.append('files', f, f.name));
  await retryKtlApiCall<KtlApiResponseData>(
    () => axios.post<KtlApiResponseData>(`${KTL_API_BASE_URL}${UPLOAD_FILES_ENDPOINT}`, formData, {
      headers: { 'Content-Type': 'multipart/form-data' },
      timeout: KTL_API_TIMEOUT,
    }),
    2, 2000, '추가사진자료 Upload'
  );

  onProgress?.('KTL 메타 데이터 전송 중...');
  // A4 파일별 개별 키 생성: 추가사진자료1, 추가사진자료2, ...
  const a4FileEntries: Record<string, string> = {};
  a4Files.forEach((f, i) => {
    a4FileEntries[`추가사진자료${i + 1}`] = f.name;
  });
  const ktlJson = {
    LABVIEW_GUBN: '추가사진자료',
    LABVIEW_DESC: JSON.stringify({ comment: `추가 사진자료 (접수번호: ${receiptNumber})` }),
    ...
    LABVIEW_ITEM: JSON.stringify({
      ...
      '현장': siteLocation,
      '사진수': photos.length,
      ...a4FileEntries,          // 추가사진자료1: "접수번호_추가사진자료_A4_1.jpg", ...
      '추가ZIP사진파일': zipFile.name,
    }),
  };
  const envRes = await retryKtlApiCall<KtlApiResponseData>(
    () => axios.post<KtlApiResponseData>(`${KTL_API_BASE_URL}${KTL_JSON_ENV_ENDPOINT}`, ktlJson, {
      headers: { 'Content-Type': 'application/json', Accept: 'application/json' },
      timeout: KTL_API_TIMEOUT,
    }),
    2, 2000, '추가사진자료 /env'
  );
  return { message: envRes.data?.message || '추가 사진자료 전송 완료' };
};
