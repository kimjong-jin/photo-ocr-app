...
const parseAndSetResponseTime = (
    valueStr: string | undefined, 
    keySuffix: 'C' | '', 
    labviewItemObject: Record<string, string>
) => {
    if (!valueStr || !valueStr.trim().startsWith('[')) return;
    try {
        const values = JSON.parse(valueStr);
        if (Array.isArray(values) && values.length === 3) {
            const [sec, min, len] = values.map(v => String(v || '').trim());
            const suffixes = ['초', '분', '길이'];
            const vals = [sec, min, len];

            vals.forEach((val, index) => {
                if (val) {
                    const key = `응답시간_${suffixes[index]}${keySuffix}`;
                    labviewItemObject[key] = val;
                }
            });
        }
...
const constructPhotoLogKtlJsonObject = (
  ...
  const compositePhotoFileName = actualKtlFileNames.find(name => name.endsWith('.jpg') || name.endsWith('.jpeg') || name.endsWith('.png') && !name.endsWith('_datatable.png'));
  const zipPhotoFileName = actualKtlFileNames.find(name => name.endsWith('.zip'));
  const dataTableImageFileName = actualKtlFileNames.find(name => name.endsWith('_datatable.png'));

  const isDrinkingWater = (ANALYSIS_ITEM_GROUPS.find(g => g.label === '먹는물')?.items || []).includes(payload.item);

  payload.ocrData.forEach(entry => {
    if (entry.identifier?.startsWith('응답시간') && isDrinkingWater) {
      if (payload.item === 'TU/CL') {
          parseAndSetResponseTime(entry.value, '', labviewItemObject); // TU
          parseAndSetResponseTime(entry.valueTP, 'C', labviewItemObject); // Cl
      } else if (payload.item === 'TU') {
          parseAndSetResponseTime(entry.value, '', labviewItemObject);
      } else if (payload.item === 'Cl') {
          parseAndSetResponseTime(entry.value, 'C', labviewItemObject);
      }
    } else if (isDrinkingWater) {
        if (!entry.identifier) return;

        // Handle TU value (present for TU and TU/CL)
        if ((payload.item === 'TU' || payload.item === 'TU/CL') && typeof entry.value === 'string' && entry.value.trim()) {
        ...
        // Handle Cl value (present for Cl and TU/CL)
        const clValueSource = payload.item === 'TU/CL' ? entry.valueTP : entry.value;
        if ((payload.item === 'Cl' || payload.item === 'TU/CL') && typeof clValueSource === 'string' && clValueSource.trim()) {
            const numericValueMatch = clValueSource.match(/^-?\d+(\.\d+)?/);
            const valueToUse = numericValueMatch ? numericValueMatch[0] : null;
            if (valueToUse !== null) {
                labviewItemObject[`${entry.identifier}C`] = valueToUse;
            }
        }
    } else { // Original Logic for PhotoLog page (수질) and FieldCount
  ...
  if (dataTableImageFileName) {
    labviewItemObject["PHOTO_데이터테이블"] = dataTableImageFileName;
  }
  ...
  if (isDrinkingWater) {
      if (payload.item === 'TU/CL') {
          if (typeof payload.maxDecimalPlaces === 'number') {
              labviewItemObject["소수점"] = String(payload.maxDecimalPlaces);
          }
          if (typeof payload.maxDecimalPlacesCl === 'number') {
              labviewItemObject["소수점C"] = String(payload.maxDecimalPlacesCl);
          }
      } else if (payload.item === 'Cl') {
          if (typeof payload.maxDecimalPlaces === 'number') {
              labviewItemObject["소수점C"] = String(payload.maxDecimalPlaces);
          }
      } else { // TU
          if (typeof payload.maxDecimalPlaces === 'number') {
              labviewItemObject["소수점"] = String(payload.maxDecimalPlaces);
  ...
  } else { // Other pages use the argument
  ...
  let gubnPrefix = '수질 분석'; // 기본값 (P1: 수질 분석)
  const drinkingWaterItems = ANALYSIS_ITEM_GROUPS.find(g => g.label === '먹는물')?.items || [];
  
  if (payload.pageType === 'FieldCount') {
    gubnPrefix = '현장계수';
  } else if (drinkingWaterItems.includes(payload.item)) {
    gubnPrefix = '먹는물';
  }
  
  // Create a single formatted site location string for KTL
  let siteLocationForKtl = payload.siteLocation;
  if (gubnPrefix === '먹는물' && payload.siteLocation.includes(' / ')) {
    const parts = payload.siteLocation.split(' / ').map(p => p.trim()).filter(p => p);
    const mainSite = parts[0];
    const details = parts.slice(1).join(' / ');
    if (mainSite && details) {
      siteLocationForKtl = `${mainSite}_(${details})`;
    }
  }
  
  // For Drinking Water page, add the site location to the LABVIEW_ITEM object
  if (gubnPrefix === '먹는물') {
    labviewItemObject["현장"] = siteLocationForKtl;
  }

  const labviewDescComment = `${gubnPrefix} (항목: ${payload.item}, 현장: ${siteLocationForKtl})`;
...
export const generateStructuralKtlJsonForPreview = (
  ...
  const dateForPreview = inspectionStartDateFromUi || new Date().toISOString().split('T')[0];
  ...
  let labviewDescComment = `구조 (항목: ${mainItemNamesForDesc.join(', ')}, 현장: ${siteLocationGlobal}, 검사시작일: ${dateForPreview})`;
  const labviewDescObject = { comment: labviewDescComment };
...
export const sendBatchStructuralChecksToKtlApi = async (
  ...
  const dateForSubmission = inspectionStartDateFromUi || new Date().toISOString().split('T')[0];
  ...
  for (const receiptNo of uniqueReceiptNumbersInBatch) {
    ...
    if (photosForThisReceipt.length > 0) {
      ...
      const stampDetailsComposite = {
        ...
        // inspectionStartDate is intentionally omitted to prevent it from being stamped.
      };
        ...
        for (const photo of photosForThisReceipt) {
            try {
                ...
                const stampedPhotoDataUrl = await generateStampedImage(
                    ...
                    photo.jobReceipt,
                    siteLocationGlobal,
                    '', // Pass an empty string for inspectionDate to prevent stamping.
                    photo.jobItemName,
                    comment
                );
  ...
  payloadsForKtlService.forEach(payload => {
    payload.inspectionStartDate = dateForSubmission; 
  ...
  for (const receiptNo in jobsByReceiptNumber) {
    ...
    const labviewDescComment = `구조 (항목: ${mainItemNamesForDesc.join(', ')}, 현장: ${siteLocationGlobal}, 검사시작일: ${dateForSubmission})`;
    const labviewDescObject = { comment: labviewDescComment };
