// src/services/email/validators.ts
...
export function sanitizeAndValidateAttachments(
  attachments: { name: string; content: string }[]
): SafeAttachment[] {
  if (!attachments?.length) throw new Error('attachments required');
  ...
  for (const att of attachments) {
    ...
    if (totalBytes > MAX_TOTAL_BYTES) throw new Error('Payload too large after attachments.');

    let buf: Buffer;
    try { buf = Buffer.from(b64, 'base64'); } catch { throw new Error('Invalid base64 attachment.'); }
    if (!isLikelyImage(buf)) throw new Error('Only image attachments (JPEG/PNG) are allowed.');
