...
▶연락처: ${editableUserContact.trim()}`;
  ...
  return (
    <div className="w-full max-w-3xl bg-slate-900/60 backdrop-blur-sm border border-slate-700/50 rounded-xl shadow-lg overflow-hidden">
      <button
        type="button"
        onClick={() => setShowContent(v => !v)}
        className="w-full flex items-center justify-between px-5 py-3 hover:bg-slate-800/40 transition-colors"
      >
        <h2 className="text-sm font-semibold text-sky-400/70 tracking-widest uppercase">
          커카오톡 메시지 전송 (P5)
        </h2>
        <span className="text-xs text-slate-500">{showContent ? '▲' : '▼'}</span>
      </button>
      <div className={`overflow-hidden transition-all duration-300 ${showContent ? 'max-h-[3000px] opacity-100' : 'max-h-0 opacity-0'}`}>
        <div className="px-5 pb-5 pt-1 space-y-2 border-t border-slate-700/50">
      <form onSubmit={handleBatchSend} className="space-y-2">
        {/* 섹션 1: 메시지 템플릿 */}
        <div className="rounded-lg border border-slate-700/40 overflow-hidden">
          <button type="button" onClick={() => toggleSection('template')}
            className={`w-full flex justify-between items-center px-3 py-2.5 text-left transition-colors hover:bg-slate-800/40 ${openSections.includes('template') ? 'bg-slate-800/30' : ''}`}>
            <span className="text-sm font-semibold text-slate-200">📝 메시지 템플릿</span>
            <span className="text-xs text-slate-500">{openSections.includes('template') ? '▲' : '▼'}</span>
          </button>
          <div className={`overflow-hidden transition-all duration-300 ${openSections.includes('template') ? 'max-h-[400px] opacity-100' : 'max-h-0 opacity-0'}`}>
            <div className="px-3 pb-3 pt-2 space-y-3 border-t border-slate-700/40">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label htmlFor="kakao-year" className="block text-xs font-medium text-slate-400 mb-1">년도 <span className="text-red-400">*</span></label>
                  <input type="text" id="kakao-year" value={year} onChange={(e) => { setYear(e.target.value); setApiResponse(null); }} required disabled={isSending}
                    className="block w-full p-2 bg-slate-700 border border-slate-600 rounded-md text-slate-100 text-sm placeholder-slate-500" placeholder="예: 2025" />
                </div>
                <div>
                  <label htmlFor="kakao-test-name" className="block text-xs font-medium text-slate-400 mb-1">검사명 <span className="text-red-400">*</span></label>
                  <input type="text" id="kakao-test-name" value={testName} onChange={(e) => { setTestName(e.target.value); setApiResponse(null); }} required disabled={isSending}
                    className="block w-full p-2 bg-slate-700 border border-slate-600 rounded-md text-slate-100 text-sm placeholder-slate-500" placeholder="예: 정도검사" />
                </div>
              </div>
              <div>
                <label htmlFor="kakao-user-contact" className="block text-xs font-medium text-slate-400 mb-1">담당자 연락수 <span className="text-red-400">*</span></label>
                <input type="text" id="kakao-user-contact" value={editableUserContact} onChange={(e) => { setEditableUserContact(e.target.value); setApiResponse(null); }} required disabled={isSending}
                  className="block w-full p-2 bg-slate-700 border border-slate-600 rounded-md text-slate-100 text-sm placeholder-slate-500" placeholder="예: 010-0000-0000" />
              </div>
            </div>
          </div>
        </div>
        ...
        {/* 섹션 2: 수신 기관 */}
        <div className="rounded-lg border border-slate-700/40 overflow-hidden">
          <button type="button" onClick={() => toggleSection('recipients')}
            className={`w-full flex justify-between items-center px-3 py-2.5 text-left transition-colors hover:bg-slate-800/40 ${openSections.includes('recipients') ? 'bg-slate-800/30' : ''}`}>
            <span className="text-sm font-semibold text-slate-200">
              🏢 수신 기관 목록
              {structuredList.length > 0 && <span className="ml-1.5 text-[10px] bg-sky-600 text-white px-1.5 py-0.5 rounded-full">{structuredList.length}</span>}
            </span>
            <span className="text-xs text-slate-500">{openSections.includes('recipients') ? '▲' : '▼'}</span>
          </button>
          <div className={`overflow-hidden transition-all duration-300 ${openSections.includes('recipients') ? 'max-h-[2000px] opacity-100' : 'max-h-0 opacity-0'}`}>
            <div className="px-3 pb-3 pt-2 space-y-2 border-t border-slate-700/40">
              <div className="p-2.5 bg-slate-800/50 rounded-lg border border-slate-700/30 space-y-2">
                <h4 className="text-xs font-semibold text-slate-400">새 기관 추가</h4>
                    <div>
                ...
                <ActionButton type="button" onClick={handleAddInstitutions} variant="secondary" fullWidth icon={<PlusIcon />}
                  disabled={isSending || !newEntrySite.trim() || !newEntryDate.trim() || !newEntryPhoneNumbers.trim()}
                  className="!py-1.5 !text-xs">믹록에 추가</ActionButton>
              </div>
              {structuredList.length === 0 && <p className="text-center text-xs text-slate-500 py-2">아직 추가된 기관이 없습니다.</p>}
              {structuredList.map((item, index) => {
                const isExpanded = expandedItems.has(item.id);
                return (
                  <div key={item.id} className="bg-slate-700/60 rounded-lg border border-slate-600/40 overflow-hidden">
                    <button type="button" onClick={() => toggleItem(item.id)}
                      className="w-full flex items-center justify-between px-2.5 py-2 text-left hover:bg-slate-600/30 transition-colors">
                      <div className="flex items-center gap-2 min-w-0">
                        <span className="text-xs font-bold text-sky-400 shrink-0">#{index + 1}</span>
                        <span className="text-xs font-medium text-slate-200 truncate">{item.site || '(현장명 없음)'}</span>
                        {item.date && <span className="text-[10px] text-slate-500 truncate hidden sm:inline">{item.date}</span>}
                      ...
                      <div className="flex items-center gap-1 shrink-0">
                        ...
                        <button type="button" onClick={(e) => { e.stopPropagation(); handleRemoveInstitution(item.id); }}
                          className="p-0.5 text-slate-500 hover:text-red-400 rounded"><TrashIcon /></button>
                      </div>
                    </button>
                    {isExpanded && (
                      <div className="px-2.5 pb-2.5 pt-1.5 space-y-1.5 border-t border-slate-600/40">
                        <div><label className="block text-[10px] text-slate-500 mb-0.5">현장명</label>
                          <input type="text" value={item.site} onChange={(e) => handleUpdateInstitution(item.id, 'site', e.target.value)} className="w-full text-xs bg-slate-600 border border-slate-500 rounded p-1.5 text-slate-100" disabled={isSending} /></div>
                        <div><label className="block text-[10px] text-slate-500 mb-0.5">기간</label>
                          <input type="text" value={item.date} onChange={(e) => handleUpdateInstitution(item.id, 'date', e.target.value)} className="w-full text-xs bg-slate-600 border border-slate-500 rounded p-1.5 text-slate-100" disabled={isSending} /></div>
                        <div><label className="block text-[10px] text-slate-500 mb-0.5">도착예정시간</label>
                          <select value={item.arrivalTime} onChange={(e) => handleUpdateInstitution(item.id, 'arrivalTime', e.target.value)} className="w-full text-xs bg-slate-600 border border-slate-500 rounded p-1.5 text-slate-100" disabled={isSending}>
                            <option value="오전">오전</option><option value="오후">오후</option></select></div>
                        <div><label className="block text-[10px] text-slate-500 mb-0.5">수신자 번호 <span className="text-red-400">*</span></label>
                          <input type="text" value={item.phoneNumber} onChange={(e) => handleUpdateInstitution(item.id, 'phoneNumber', e.target.value)} className="w-full text-xs bg-slate-600 border border-slate-500 rounded p-1.5 text-slate-100" disabled={isSending} /></div>
                      </div>
                    )}
                  </div>
                );
