...
/** 공통 파일명 빌더 */
const buildBaseName = (receipt: string, site: string, item: string) => {
  const sanitizedReceipt = sanitizeFilename(receipt);
  const sanitizedSite = sanitizeFilename(site);
  const sanitizedItem = sanitizeFilename(item.replace('/', '_'));
...
const calculateConcentrationBoundariesInternal = (data: ExtractedEntry[] | null): ConcentrationBoundaries | null => {
  ...
  if (uniqueNumericValues.length < 2) {
    ...
    if (span > 0) {
      ...
      if (b1 >= b2) {
        const N = uniqueNumericValues.length;
        let idx1 = Math.max(0, Math.floor(N / 3) - 1);
        let idx2 = Math.max(idx1 + 1, Math.floor((2 * N) / 3) - 1);
        idx2 = Math.min(N - 2, idx2);
        idx1 = Math.min(idx1, Math.max(0, idx2 - 1));
        if (idx1 >= 0 && idx1 < idx2 && idx2 < N && uniqueNumericValues[idx1] < uniqueNumericValues[idx2]) {
      ...
      if (b2 < overallMax) {
        ...
        if (b1 === b2 && b1 > overallMin) {
          ...
          for (let i = uniqueNumericValues.length - 1; i >= 0; i--) {
            if (uniqueNumericValues[i] < b1) { prevValIndex = i; break; }
          }
...
const generateIdentifierSequence = (ocrData: ExtractedEntry[] | null, currentSelectedItem: string): string => {
  if (!ocrData) return '';
  let sequence = '';
...
const PhotoLogPage: React.FC<PhotoLogPageProps> = ({ userName, jobs, setJobs, activeJobId, setActiveJobId, siteLocation, onDeleteJob }) => {
...
컨텍스트:`;
    ...
    if (item === 'TN/TP') {
      ...
      prompt += `\n2.  한 값만 있는 경우: 특정 시간대에 TN 또는 TP 값 중 하나만 명확하게 식별 가능한 경우 해당 값만 포함합니다.`;
      prompt += `\n3.  값 형식: 모든 값 필드에는 이미지에서 보이는 순수한 숫자 값만 포함해야 합니다.`;
      prompt += `\n\nJSON 출력 형식 예시 (항목: TN/TP):\n[\n  { "time": "2025/04/23 05:00", "value_tn": "46.2", "value_tp": "1.2" },\n  { "time": "2025/04/23 06:00", "value_tn": "5.388", "value_tp": "0.882" }\n]`;
    } else {
      prompt += `\n- 항목/파라미터: ${item}. 이 항목의 측정값을 이미지에서 추출해주세요.`;
      prompt += `\n"value" 필드에는 각 측정 항목의 순수한 숫자 값만 포함해야 합니다.`;
...
JSON 출력 및 데이터 추출을 위한 특정 지침: 반드시 유효한 단일 JSON 배열이어야 하며, 기타 텍스트는 포함하지 마세요.`;
    return prompt;
  };
  ...
  }, [activeJob, userName, ktlJsonPreview, hypotheticalKtlFileNamesForPreview]);
  ...
  }, [activeJob, userName, updateActiveJob]);
    ...
    for (let i = 0; i < jobsToSend.length; i++) {
      ...
      try {
        ...
        const payload: ClaydoxPayload = {
          ...
          updateUser: userName,
          ocrData: job.processedOcrData!, // 존재 보장
          identifierSequence,
  ...
  const handleDownloadStampedImages = useCallback(async () => {
    ...
    try {
      ...
      for (let i = 0; i < activeJob.photos.length; i++) {
        ...
        zip.file(`${baseName}_${i + 1}.${outExt}`, blob);
      }
      const zipBlob = await zip.generateAsync({ type: 'blob' });
  ...
  return (
    <div className="w-full max-w-4xl bg-slate-800 shadow-2xl rounded-xl p-6 sm:p-8 space-y-6">
      <h2 className="text-2xl font-bold text-sky-400 border-b border-slate-700 pb-3">수질 분석 (P3)</h2>
