export type MainStructuralItemKey = 'TOC' | 'TN' | 'TP' | 'SS' | 'PH' | 'DO' | 'COD' | 'TU' | 'Cl';
...
export const MAIN_STRUCTURAL_ITEMS: { key: MainStructuralItemKey; name: string }[] = STRUCTURAL_ITEM_GROUPS.flatMap(group => group.items);
...
export const MEASUREMENT_METHOD_OPTIONS: Partial<Record<MainStructuralItemKey, string[]>> = {
  ...
  Cl: ["고시에 명시된 측정방법 확인", OTHER_DIRECT_INPUT_OPTION],
};
