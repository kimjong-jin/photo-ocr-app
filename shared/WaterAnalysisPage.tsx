...
import { FileWithContent, KtlWaterAnalysisPayload } from '../src/types';
import { uploadZipFile, sendWaterAnalysisData } from '../src/services/ktlApiService';
...
const WaterAnalysisPage: React.FC<WaterAnalysisPageProps> = ({ userName, userRole }) => {
    ...
    const processGroup = useCallback(async (receiptNumber: string, rowsInGroup: WaterAnalysisRow[], currentFileVersion: string | null) => {
        ...
        try {
            ...
            throw groupError; // re-throw to be caught by the main processor
        }
    }, [excelFile, pdfFiles, imageFiles, log, createPayloadForRowGroup]);
    
    const processAndSend = async () => {
        if (!excelFile) { alert("엑셀 파일을 선택해주세요."); return; }
        if (userRole === "guest") { alert("게스트 사용자는 자료를 전송할 수 없습니다."); return; }
