...
const MAX_FILES = 15;
const MAX_SINGLE_BYTES = 4 * 1024 * 1024;  // 4MB (base64 효 ~5.3MB, Vercel 4.5MB 한도 먼저 차단)
const MAX_TOTAL_BYTES  = 5 * 1024 * 1024;  // 5MB (UI 기준)
...
// 이미지만 축소(총합 2MB 목표 → base64 ~2.7MB, Vercel 4.5MB 한도 안전마진)
async function shrinkImagesToMaxSize(images: ImageInfo[], maxTotalBytes = 2_000_000) {
...
const EmailModal: React.FC<Props> = ({ isOpen, onClose, application, userName, onSendSuccess }) => {
  ...
  const initialBody = useMemo(() => {
    const phoneDigits = ((application as any)?.applicant_phone || '').replace(/\D/g, '');
    const pw = phoneDigits.length >= 8 ? phoneDigits.slice(-8) : phoneDigits || '(전화번호 없음)';
    const lines = [
      ...
      ``,
      `시험·검사 관련 자료(사진/기록부)를 암호화 PDF 파일로 첨부드립니다.`,
      ``,
      `※ PDF 비밀번호: 담당자 또는 유지관리담당자 휴대전화번호 뒷 8자리 (하이픈 제외)`,
      `   예) 010-1234-5678 → 비밀번호: 12345678`,
      `※ 비밀번호를 10회 이상 잘못 입력하면 파일이 잠길 수 있습니다.`,
      ``,
      `문의 사항은 담당자에게 연락해 주시기 바랍니다.`,
      ``,
      `※ 본 메일은 발신 전용(no-reply) 주소에서 발송되었습니다. 회신 메일은 확인되지 않습니다.`,
    ].filter(l => l !== undefined);
    return lines.join('\n');
  }, [userName, application?.receipt_no, application?.site_name, (application as any)?.applicant_phone]);
  ...
  const handleFilePick = async (files: FileList | null) => {
    ...
    for (const file of Array.from(files)) {
      ...
      // ThumbnailGallery는 순수 base64를 기대하므로 data: 접두어 제거
  ...
  const handleCameraCapture = (file: File, base64: string, mimeType: string) => {
    ...
    // 카메라 base64도 순수 base64로 정규화
    const pureBase64 = base64.includes(',') ? base64.split(',')[1] : base64;
    setImages((prev) => {
      const next = [...prev, { file, base64: pureBase64, mimeType, name: sanitizeFilename(file.name) }];
  ...
  // 이미지 1장 → 암호화 PDF base64 변환
  const imageToPdfBase64 = async (
    img: { base64: string; name?: string },
    password: string
    ...
    return new Promise((resolve, reject) => {
      ...
      image.onload = async () => {
        try {
          ...
          const encBytes = await pdfDoc.save();
          // btoa 변환: 대용량 파일에서 스프레드 연산자 콜스택 오버플로우 방지
          const uint8Array = new Uint8Array(encBytes);
          const CHUNK = 8192;
          let binaryStr = '';
          ...
          const baseName = (img.name || 'photo').replace(/\.[^.]+$/, '');
          resolve({ name: `${baseName}.pdf`, content: b64 });
        } catch (e) { reject(e); }
      };
      image.onerror = reject;
      image.src = img.base64.startsWith('data:') ? img.base64 : `data:image/jpeg;base64,${img.base64}`;
    });
  };
  ...
  const handleSend = async () => {
    ...
    // 비밀번호: 휴대폰 뒷 8자리
    const phoneDigits = ((application as any)?.applicant_phone || '').replace(/\D/g, '');
    const password = phoneDigits.length >= 8 ? phoneDigits.slice(-8) : phoneDigits;
    if (!password) return setStatus({ type: 'error', text: '신청자 전화번호가 없어 PDF 비밀번호를 생성할 수 없습니다.' });
    ...
    try {
      setStatus({ type: 'info', text: `이미지 압축 중... (${images.length}장)` });

      // 1-a) 이미지 크기 최적화 (2MB 목표 → base64 ~2.7MB, Vercel 4.5MB 한도 안전)
      const shrunkImages = images.length > 0
        ? await shrinkImagesToMaxSize(images, 2_000_000)
        : [];

      setStatus({ type: 'info', text: `이미지 → 암호화 PDF 변환 중... (${shrunkImages.length}장)` });

      // 1-b) 압쳐진 이미지 → 개별 암호화 PDF 변환
      const imagePdfs = await Promise.all(
        shrunkImages.map((img) => imageToPdfBase64(img, password))
      );

      // 2) 기존 PDF 첨부 추가 (원본 그대로)
      const existingPdfs = pdfs.map(p => ({
        name: p.name || 'document.pdf',
        content: p.base64.startsWith('data:') ? p.base64.split(',')[1] : p.base64,
      }));

      const allAttachments = [...imagePdfs, ...existingPdfs];

      setStatus({ type: 'info', text: `전송 중... (PDF ${allAttachments.length}개)` });
        ...
        <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-6 overflow-y-auto pr-1">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-semibold text-slate-100">첨부(이미지/PDF, 최대 {MAX_FILES}개)</h3>
              <div className="flex gap-2">
                {isSending && (
            ...
            {isCameraOpen ? (
              ...
              <div>
                ...
                />
                <p className="mt-2 text-xs text-slate-400">이미지는 자동 최적화(이미지 합계 2MB 목표), PDF는 원본 유지. (단일 4MB / 합계 5MB 권장)</p>
              </div>
            )}
            ...
            {pdfs.length > 0 && (
              <div className="bg-slate-700/40 rounded-md p-3">
                <div className="text-sm font-semibold text-slate-100 mb-2">PDF 첨부</div>
