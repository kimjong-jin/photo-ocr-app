...
const MapView: React.FC<MapViewProps> = ({ latitude, longitude, onAddressSelect, savedLocations }) => {
  ...
  useEffect(() => {
    ...
    const init = () => {
      ...
      if (!naverMapRef.current) {
        ...
        // 클릭 → 핀 이동 + 주소 역지오코딩 (카카오와 동일)
        nmaps.Event.addListener(naverMapRef.current, 'click', (e: any) => {
          const p = e.coord;
  ...
  // 구글 지도(JS SDK) — 'google' 선택 시. 카카오처럼 클릭 → 핀 이동 + 주소 역지오코딩.
  useEffect(() => {
    if (mapProvider !== 'google') return;
    const key = import.meta.env.VITE_GOOGLE_MAPS_KEY;
    if (!key) { console.warn('[MapView] VITE_GOOGLE_MAPS_KEY 미설정'); return; }
    const lat = latitude || DEFAULT_LAT, lng = longitude || DEFAULT_LNG;
    const init = () => {
    ...
    let s = document.getElementById('google-map-script') as HTMLScriptElement | null;
    if (!s) {
      s = document.createElement('script');
