...
interface OcrControlsProps {
  ...
  loadDraftMessage?: string | null;
  loadDraftReceiptNumber?: string | undefined | null;
  showSaveDraftWarning?: boolean;
}
...
export const OcrControls: React.FC<OcrControlsProps> = ({ 
  ...
  saveDraftMessage,
  loadDraftMessage,
  loadDraftReceiptNumber,
  showSaveDraftWarning
}) => {

  const isActionDisabled = isClearDisabled || isSavingDraft || isLoadingDraft;
          ...
          {onSaveDraft && (
            <div className="space-y-1">
              <ActionButton
                ...
                icon={isSavingDraft ? <Spinner size="sm" /> : <SaveIcon />}
                fullWidth
                variant="secondary"
              >
              ...
              {showSaveDraftWarning && (
                <p className="text-xs text-center text-slate-500">이미지 사진은 저장되지 않습니다.</p>
              )}
              {saveDraftMessage && (
                <p className="text-xs text-center text-green-400" role="status">✅ {saveDraftMessage}</p>
              )}
            </div>
          )}
          {onLoadDraft && (
            <div className="space-y-1">
              <ActionButton
                onClick={onLoadDraft}
                disabled={isActionDisabled}
                icon={isLoadingDraft ? <Spinner size="sm" /> : <LoadIcon />}
                fullWidth
                variant="secondary"
              >
              ...
              {loadDraftReceiptNumber && (
                <p className="text-xs text-center text-slate-400">대상: {loadDraftReceiptNumber}</p>
              )}
              {loadDraftMessage && (
                 <p className="text-xs text-center text-green-400" role="status">✅ {loadDraftMessage}</p>
              )}
            </div>
          )}
        </div>
