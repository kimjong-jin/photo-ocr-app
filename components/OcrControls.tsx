...
interface OcrControlsProps {
  ...
  onInitiateSendToKtl?: () => void; // 변경: onSendToClaydox -> onInitiateSendToKtl
    ...
    <div className="space-y-4 pt-2">
      {showNavigation && onPreviousImage && onNextImage && (
        <div className="flex items-center justify-between">
          ...
          <span className="text-sm text-slate-400">
            이미지 {currentImageIndex! + 1} / {totalImages}
          </span>
          <ActionButton 
            onClick={onNextImage} 
      ...
      {onInitiateSendToKtl && ( // 변경됨
        <div className="space-y-1">
         <ActionButton
            onClick={onInitiateSendToKtl} // 변경됨
            ...
            aria-label="데이터 및 이미지를 KTL API로 전송 전 확인" // 레이블 업데이트
        >
