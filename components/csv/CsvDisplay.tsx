...
const MiniMap: React.FC<MiniMapProps> = ({
  ...
  const mapX = useCallback(
    (ts: number) => ((ts - fullTimeRange.min) / (fullTimeRange.max - fullTimeRange.min)) * width,
    [width, fullTimeRange]
  );
  const unmapX = useCallback(
    (x: number) => fullTimeRange.min + (x / width) * (fullTimeRange.max - fullTimeRange.min),
    [width, fullTimeRange]
  );
  ...
  useEffect(() => {
    ...
    const channelData = fullData
      .map(d => ({ ts: d.timestamp.getTime(), v: d.values[channelIndex] }))
      .filter(d => d.v !== null) as { ts: number; v: number }[];
  ...
  const handlePointerMove = (e: React.PointerEvent) => {
    ...
    if (isDragging === 'move') {
      const nextEnd = Math.min(
        fullTimeRange.max,
        Math.max(fullTimeRange.min + dragData.current.startRange, dragData.current.startEndTs + deltaTs)
      );
      onNavigate(nextEnd);
...
const GraphCanvas: React.FC<GraphCanvasProps> = ({
  ...
  onAiPointChange,
  placingAiPointLabel,
  onManualAiPointPlacement,
  setPlacingAiPointLabel,
  isRangeSelecting,
  rangeSelection,
  onRangeSelectComplete,
  sequentialPlacementState,
  onSequentialPointPlacement,
  sensorType,
  ...
  const handlePointerDown = (e: React.PointerEvent) => {
    ...
    longPressTimerRef.current = window.setTimeout(() => {
      ...
      touchState.current.hasMoved = true; // 롱프레스 들어가면 클릭 방지
      if ('vibrate' in navigator) navigator.vibrate(30);
    }, 1000);

    // 마커 히트 테스트 우선
    let markerHit: { label: string } | null = null;
    if (aiAnalysisResult) {
      Object.entries(aiAnalysisResult).forEach(([key, pt]) => {
        if (pt && typeof pt === 'object' && (pt as any).timestamp) {
          ...
          if (Math.abs(x - px) < 20 && Math.abs(y - py) < 20) markerHit = { label: key };
        }
      });
    }
    ...
    // 기본 팬: 포인트 지정/순차/최대최소 모드가 아닐 때 즉시 팬
  ...
  const handlePointerMove = (e: React.PointerEvent) => {
    ...
    if (draggingPoint) {
      ...
      let closest: { timestamp: Date; value: number } | null = null;
      ...
      if (closest) {
        onAiPointChange(draggingPoint.label, {
          timestamp: closest.timestamp.toISOString(),
          value: closest.value,
        });
      }
      return;
    ...
    if (x >= padding.left && x <= width - padding.right && getChannelData.length > 0) {
      ...
      setCrosshairPos({
        x: mapX(closest.timestamp.getTime()),
        y: mapY(closest.value),
  ...
  const handleClick = (e: React.MouseEvent) => {
    ...
    const timeAtMouse = viewportMin + ((x - padding.left) / graphWidth) * (viewportMax - viewportMin);
    const currentLabel =
      placingAiPointLabel || (sequentialPlacementState.isActive ? SEQUENTIAL_POINT_ORDER[sequentialPlacementState.currentIndex] : null);
    ...
    if (currentLabel?.toUpperCase() === 'EN' && (sensorType === 'PH' || sensorType === 'DO')) {
      ...
      let interpPoint: { timestamp: Date; value: number } | null = null;
  ...
  useEffect(() => {
    ...
    ctx.font = '10px Inter';
    for (let i = 0; i <= 5; i++) {
      const yy = padding.top + (i / 5) * graphHeight;
      const val = getYBounds.yMax - (i / 5) * (getYBounds.yMax - getYBounds.yMin);
      ctx.beginPath();
      ctx.moveTo(padding.left, yy);
      ctx.lineTo(padding.left + graphWidth, yy);
      ctx.stroke();
      ctx.textAlign = 'right';
      ctx.fillText(val.toFixed(2), padding.left - 8, yy + 3);
    }
    ...
    if (aiAnalysisResult) {
      ...
      if (st?.timestamp && en?.timestamp) {
        ...
        if (sx >= padding.left && sx <= width - padding.right) {
          ...
          ctx.lineTo(sx, padding.top + graphHeight);
          ctx.stroke();
          ctx.restore();
        }
        ...
        if (ex >= padding.left && ex <= width - padding.right) {
          ...
          ctx.lineTo(ex, padding.top + graphHeight);
          ctx.stroke();
          ctx.restore();
        }
      }
    ...
    getChannelData.forEach((d, i) => {
      const px = mapX(d.timestamp.getTime());
      const py = mapY(d.value);
    ...
    if (crosshairPos) {
      ...
      ctx.lineTo(crosshairPos.x, padding.top + graphHeight);
      ctx.stroke();
      ...
      ctx.fillText(txt, crosshairPos.x, padding.top - 12);
      ctx.restore();
    }
    ...
    if (isMaxMinMode) {
      ...
      ctx.textAlign = 'center';
      ctx.fillText(
        selection?.start ? '두 번째 지점을 클릭하여 범위를 완료하세요' : '최대/최소 분석: 첫 번째 지점을 클릭하세요',
        width / 2,
        padding.top / 2 + 5
      );
      ctx.restore();
...
export const CsvDisplay: React.FC<CsvDisplayProps> = props => {
  const {
    ...
    toggleAnalysisMode,
    toggleMaxMinMode,
    handleDeleteManualResult,
    handleResetAnalysis,
    handlePointSelect,
    handleAiPointChange,
    placingAiPointLabel,
    setPlacingAiPointLabel,
    handleManualAiPointPlacement,
    isFullScreenGraph,
    setIsFullScreenGraph,
    aiPointHistory,
    handleUndoAiPointChange,
    sequentialPlacementState,
    handleToggleSequentialPlacement,
    handleSequentialPointPlacement,
    SEQUENTIAL_POINT_ORDER,
  } = props;
  ...
  useEffect(() => {
    ...
    if (activeJob.aiAnalysisResult) {
      ...
      if (st && en) {
        ...
        results.push({
          ...
          type: '응답',
          name: 'ST → EN',
          startTime: new Date(st.timestamp),
          endTime: new Date(en.timestamp),
          diff: diffSec,
        });
      }
      ...
      Object.entries(activeJob.aiAnalysisResult).forEach(([key, point]) => {
        if (point && typeof point === 'object' && (point as any).timestamp) {
          ...
          results.push({
            ...
            startTime: new Date((point as any).timestamp),
    ...
    if (selectedChannel) {
      (activeJob.channelAnalysis[selectedChannel.id]?.results || []).forEach((res: any, idx: number) => {
        results.push({
          id: res.id,
          type: '수동 분석',
  ...
  const getSensorPoints = (type: SensorType) => {
    switch (type) {
      ...
      case 'PH':
        return [
          '(A)_4_1', '(A)_4_2', '(A)_4_3',
          '(A)_7_1', '(A)_7_2', '(A)_7_3',
          '(A)_10_1', '(A)_10_2', '(A)_10_3',
          '(B)_7_1', '(B)_4_1', '(B)_7_2', '(B)_4_2', '(B)_7_3', '(B)_4_3',
          '(C)_4_1', '(C)_4_2', '(C)_4_3',
          '(C)_7_1', '(C)_7_2', '(C)_7_3',
          '(C)_4_4', '(C)_4_5', '(C)_4_6',
          '(C)_7_4', '(C)_7_5', '(C)_7_6',
          '4_10', '4_15', '4_20', '4_25', '4_30',
          'ST', 'EN', '현장1', '현장2',
        ];
      case 'DO':
        return [
          ...
          'S_4', 'S_5', 'S_6',
          '20_S_1', '20_S_2', '20_S_3',
          '30_S_1', '30_S_2', '30_S_3',
          'ST', 'EN',
        ];
      default:
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <button
                ...
                className={`py-3 rounded-md text-white text-sm transition-colors ${
                  isManualActive ? 'bg-sky-600 ring-2' : 'bg-slate-700 hover:bg-slate-600'
                }`}
              >
                1. 수동 분석
              </button>
            ...
            <button
              ...
              className={`w-full py-2.5 rounded-md text-white text-sm transition-colors ${
                activeJob.isMaxMinMode ? 'bg-amber-600 ring-2' : 'bg-slate-700 hover:bg-slate-600'
              }`}
            >
              최대/최소 (범위 지정)
            </button>
            ...
            {isManualActive && (
              <div className="pt-2 border-t border-slate-700 space-y-3">
                <div className="flex flex-wrap gap-1.5">
                  {(['SS', 'PH', 'TU', 'Cl', 'DO'] as SensorType[]).map(opt => (
                    <button
                      key={opt}
                      onClick={() => updateActiveJob(j => ({ ...j, sensorType: opt }))}
                      className={`px-2 py-1 rounded text-[10px] transition-colors ${
                        activeJob.sensorType === opt ? 'bg-sky-500 text-white' : 'bg-slate-700 text-slate-300'
                      }`}
                    >
                      {opt}
                    </button>
                ...
                <div className="flex gap-2">
                  ...
                  <ActionButton
                    onClick={handleUndoAiPointChange}
                    disabled={aiPointHistory.length === 0}
                    fullWidth
                    variant="secondary"
                    className="!text-[10px] py-1"
                  >
                ...
                <div className="grid grid-cols-3 gap-1.5 max-h-[320px] overflow-y-auto pr-1">
                  {getSensorPoints(activeJob.sensorType).map(p => (
                    <button
                      ...
                      className={`text-[10px] font-bold rounded px-1 py-2 transition-colors truncate ${
                        placingAiPointLabel === p
        ...
        <div className="space-y-4 lg:w-2/3 flex flex-col">
          ...
          <div className="flex justify-between items-center bg-slate-900/50 p-4 rounded-lg border border-slate-700">
              ...
              {activeJob.parsedData!.channels.map(ch => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded text-xs font-medium transition-colors ${
                    activeJob.selectedChannelId === ch.id ? 'bg-sky-500 text-white' : 'bg-slate-700 text-slate-300'
                  }`}
                >
                  {ch.name}
                </button>
            ...
            <div className="flex items-center bg-slate-700/50 p-1 rounded-lg">
              {[10, 30, 60, 180, 'all'].map(m => {
                const val = m === 'all' ? 'all' : Number(m) * 60 * 1000;
                return (
                  <button
                    key={String(m)}
                    onClick={() => handleTimeRangeChange(val as any)}
                    className={`px-2 py-1 rounded text-[10px] ${activeJob.timeRangeInMs === val ? 'bg-sky-500' : ''}`}
                  >
                    {m === 'all' ? '전체' : `${m}분`}
                  </button>
                );
          ...
          <div className="relative bg-slate-900 rounded-xl overflow-hidden border border-slate-700 h-[400px]">
            <button
              ...
              className="absolute top-2 right-2 z-20 p-2 text-slate-400 hover:text-white bg-slate-800/80 rounded-full"
            >
              <EnterFullScreenIcon />
            </button>
          ...
          <tbody className="divide-y divide-slate-700">
            {unifiedResults.map(item => (
              <tr key={item.id} className={`hover:bg-slate-700/30 text-slate-300 ${item.type === '응답' ? 'bg-amber-900/20' : ''}`}>
                ...
                <td className="px-3 py-2 text-right text-slate-400">{item.max?.toFixed(3) || '-'}</td>
                <td className="px-3 py-2 text-right text-slate-400">{item.min?.toFixed(3) || '-'}</td>
