...
interface OcrResultDisplayProps {
  ...
  // 현장 계수 별도 분석 결과 (P2에서 추가 분석 시)
  fieldCountData?: ExtractedEntry[] | null;
  fieldCountAnalyzedAt?: string;
  fieldCountIdentifiers?: string[];          // 현장 계수용 식별자 목록
  onFieldCountIdentifierChange?: (entryId: string, identifier: string) => void;
}
...
export const OcrResultDisplay: React.FC<OcrResultDisplayProps> = ({ 
    ...
    decimalPlaces,
    fieldCountData,
    fieldCountAnalyzedAt,
    fieldCountIdentifiers,
    onFieldCountIdentifierChange,
}) => {
  ...
  if (isLoading) {
    return (
      <div className="mt-6 p-6 bg-slate-800/50 rounded-lg border border-slate-700/30 shadow flex flex-col items-center justify-center min-h-[10rem]">
        <Spinner size="lg" />
  ...
  if (!contextProvided) {
    return (
      <div className="mt-6 p-6 bg-slate-800/50 rounded-lg border border-slate-700/30 shadow text-center">
        ...
        {ocrData.length > 0 && (
            ...
            <table className={`min-w-full divide-y divide-slate-700 ${!isManualEntryMode ? 'table-fixed' : 'table-auto'}`}>
                <thead className="bg-slate-700/50">
                    <tr>
                        <th scope="col" className={`px-1 py-3 text-center text-xs font-medium text-slate-300 uppercase tracking-wider ${!isManualEntryMode ? 'w-[4%]' : 'w-8'}`}>No.</th>
                        <th scope="col" className={`px-3 py-3 text-center text-xs font-medium text-slate-300 uppercase tracking-wider ${!isManualEntryMode ? (showTwoValueColumns ? 'w-[28%]' : 'w-[40%]') : 'w-48'}`}>{timeColumnHeader || "측정 시간"}</th>
                        {showTwoValueColumns ? (
                        <>
                            <th scope="col" className={`px-3 py-3 text-center text-xs font-medium text-slate-300 uppercase tracking-wider ${!isManualEntryMode ? (isTnTpMode ? 'w-[16%]' : 'w-[20%]') : 'w-32'}`}>{isTnTpMode ? 'TN 값' : 'TU 값'}</th>
                            <th scope="col" className={`px-3 py-3 text-center text-xs font-medium text-slate-300 uppercase tracking-wider ${!isManualEntryMode ? (isTnTpMode ? 'w-[16%]' : 'w-[20%]') : 'w-32'}`}>{isTnTpMode ? 'TP 값' : 'Cl 값'}</th>
                        </>
                        ) : (
                            <th scope="col" className={`px-3 py-3 text-center text-xs font-medium text-slate-300 uppercase tracking-wider ${!isManualEntryMode ? 'w-[28%]' : 'w-32'}`}>값</th>
                        )}
                        <th scope="col" className={`px-2 py-3 text-center text-xs font-medium text-slate-300 uppercase tracking-wider ${!isManualEntryMode ? (showTwoValueColumns ? 'w-[14%]' : 'w-[22%]') : 'w-40'}`}>{isManualEntryMode ? '구분' : (isTnTpMode ? 'TN 식별자' : '식별자')}</th>
                        {isTnTpMode && !isManualEntryMode && <th scope="col" className={`px-2 py-3 text-center text-xs font-medium text-slate-300 uppercase tracking-wider ${!isManualEntryMode ? 'w-[14%]' : 'w-40'}`}>TP 식별자</th>}
                        {onDeleteEntry && <th scope="col" className="px-1 py-3 text-center text-xs font-medium text-slate-300 uppercase tracking-wider w-8">삭제</th>}
                    </tr>
                </thead>
                ...
                {ocrData.map((entry, index) => {
                    ...
                    <tr key={entry.id} className={`${isSequenceRow ? 'bg-slate-900' : 'hover:bg-slate-700/30'} transition-colors duration-100`}>
                        ...
                        <td className={`px-1 align-top ${isSequenceRow ? 'py-0.5' : 'py-1'}`}>
                            {isManualEntryMode ? (
                              <span className="block w-full px-2 py-1.5 text-xs bg-slate-900/40 rounded-md border border-slate-700/50 select-none leading-snug">
                                <span className="block text-slate-500">{entry.time.split(' ')[0] || '—'}</span>
                                {entry.time.split(' ').slice(1).join(' ') && (
                                  <span className="block text-sky-400/70 font-mono">{entry.time.split(' ').slice(1).join(' ')}</span>
                                )}
                              </span>
                            ) : (() => {
                              // 날짜 + 시간 분리
                              const spaceIdx = entry.time.indexOf(' ');
                              const datePart = spaceIdx >= 0 ? entry.time.slice(0, spaceIdx) : entry.time;
                              const timePart = spaceIdx >= 0 ? entry.time.slice(spaceIdx + 1) : '';
                              const handleDateChange = (newDate: string) => {
                                onEntryTimeChange(entry.id, timePart ? `${newDate} ${timePart}` : newDate);
                              };
                              const handleTimeChange = (newTime: string) => {
                                onEntryTimeChange(entry.id, datePart ? `${datePart} ${newTime}` : newTime);
                              };
                              return (
                                <div className="flex flex-col gap-0.5">
                                  <input
                                    ...
                                    placeholder="날짜"
                                    className="w-full bg-slate-700 px-1.5 py-1 border border-slate-600 rounded text-xs text-slate-300 focus:ring-1 focus:ring-sky-500 focus:border-sky-500 focus:outline-none"
                                    aria-label={`날짜 입력 ${index + 1}`}
                                  />
                                  <input
                                    type="text"
                                    value={timePart}
                                    ...
                                    placeholder="시간"
                                    className="w-full bg-slate-700 px-1.5 py-1 border border-slate-600 rounded text-xs text-sky-300 font-mono focus:ring-1 focus:ring-sky-500 focus:border-sky-500 focus:outline-none"
                                    aria-label={`시간 입력 ${index + 1}`}
                                  />
                                </div>
                              );
                            })()}
                        ...
                        {onDeleteEntry && (
                            <td className={`px-1 whitespace-nowrap text-sm text-center align-top ${isSequenceRow ? 'py-1' : 'py-2'}`}>
                                <button
                                    onClick={() => onDeleteEntry(entry.id)}
                                    className="p-1 text-slate-500 hover:text-red-400 rounded transition-colors hover:bg-slate-700"
                ...
                {/* ── 현장 계수 추가 분석 결과 ── */}
                {fieldCountData && fieldCountData.length > 0 && (() => {
                  const colCount = 3 + (showTwoValueColumns ? 1 : 0) + (isTnTpMode && !isManualEntryMode ? 1 : 0) + (onDeleteEntry ? 1 : 0);
                  return (
                    <tbody className="bg-slate-800 divide-y divide-slate-700/50">
                      {/* 구분선 행 */}
                      <tr>
                        <td colSpan={colCount} className="py-3 px-2 bg-slate-700/20">
                          <div className="flex items-center gap-2">
                            <div className="flex-grow border-t-2 border-amber-500/50"></div>
                            <span className="px-3 py-0.5 rounded-full bg-amber-500/20 text-amber-400 text-xs font-bold tracking-wider whitespace-nowrap border border-amber-500/30">
                              ▼ 현장 계수 추가 분석 {fieldCountAnalyzedAt ? `(${fieldCountAnalyzedAt})` : ''}
                            </span>
                            <div className="flex-grow border-t-2 border-amber-500/50"></div>
                          </div>
                        </td>
                      </tr>
                      {fieldCountData.map((entry, idx) => (
                        <tr key={`fc-${entry.id}`} className="hover:bg-slate-700/20 transition-colors">
                          <td className="px-2 py-2 text-sm text-amber-400/70 text-center">{(ocrData?.length ?? 0) + idx + 1}</td>
                          <td className="px-2 py-2 text-sm text-slate-300 text-center">{entry.time || '—'}</td>
                          {showTwoValueColumns ? (
                            <>
                              <td className="px-2 py-2 text-sm text-slate-200 text-center font-mono">{entry.value || '—'}</td>
                              <td className="px-2 py-2 text-sm text-slate-200 text-center font-mono">{entry.valueTP || '—'}</td>
                            </>
                          ) : (
                            <td className="px-2 py-2 text-sm text-slate-200 text-center font-mono">{entry.value || '—'}</td>
                          )}
                          <td className="px-2 py-2 text-xs text-center">
                            {onFieldCountIdentifierChange ? (
                              <input
                                ...
                                onChange={(e) => onFieldCountIdentifierChange(entry.id, e.target.value)}
                                placeholder="현장1"
                                className={`w-full bg-slate-700 px-1.5 py-1 border border-slate-600 rounded text-xs focus:ring-1 focus:ring-amber-500 focus:border-amber-500 focus:outline-none ${entry.identifier ? 'text-amber-400 font-bold' : 'text-slate-400'}`}
                              />
                            ) : (
                              <span className="text-amber-400">{entry.identifier || '—'}</span>
                            )}
                          </td>
                          {isTnTpMode && !isManualEntryMode && <td className="px-2 py-2 text-xs text-amber-400 text-center">{entry.identifierTP || '—'}</td>}
                          {onDeleteEntry && <td></td>}
                        </tr>
                      ))}
                    </tbody>
