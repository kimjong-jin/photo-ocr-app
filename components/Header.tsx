...
export const Header: React.FC<HeaderProps> = ({ apiMode, onApiModeChange }) => {
    ...
    const currentModeText = apiMode === 'gemini' ? '외부 AI (Gemini)' : '내부 AI (vLLM)';
    const buttonText = apiMode === 'gemini' ? '전환 → vLLM' : '전환 → Gemini';
    ...
    return (
        <header className="w-full max-w-3xl mb-6 sm:mb-8 text-center">
            <div className="flex justify-between items-center w-full mb-4 px-1">
                <span className="text-slate-300 font-semibold text-sm">{currentModeText}</span>
                <button 
