  ...
  if (!env) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-slate-900 text-slate-300 p-8">
        <AlertTriangleIcon />
        <h2 className="text-2xl font-bold mb-2 text-red-400">빌드 환경 오류</h2>
        <p className="text-center max-w-md">
          <code>import.meta.env</code>가 정의되지 않았습니다. 이 앱은 Vite와 같은 빌드 도구를 사용하여 빌드해야 합니다.
        </p>
        <p className="text-sm mt-4 text-slate-500">이 오류는 앱을 빌드하면 해결됩니다.</p>
      </div>
    );
  }
