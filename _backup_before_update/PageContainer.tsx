...
const PageContainer: React.FC<PageContainerProps> = ({ userName, userRole, userContact, onLogout }) => {
  ...
  const handleSaveDraft = useCallback(async (receipt?: string) => {
    const receiptToSave = receipt || getReceiptNumberForSaveLoad();
    ...
    try {
      ...
      const makePageUpload = (
        ...
        const photoTasks = jobs.flatMap(job =>
        ...
        if (photoTasks.length === 0) return; // 사진 없으면 건드리지 않음
        deletePhotosFromServer(receiptToSave, pageCode, userName)
          .catch(() => {})
          .then(() => Promise.all(photoTasks))
          .then(() => setDraftMessage({ type: 'success', text: `'${receiptToSave}' ${pageCode} 사진 ${photoTasks.length}장 저장 완료` }));
      };

      const p1Jobs = structuralCheckJobs.filter(j => j.receiptNumber === receiptToSave);
      const p2Jobs = photoLogJobs.filter(j => j.receiptNumber === receiptToSave);
      const p3Jobs = fieldCountJobs.filter(j => j.receiptNumber === receiptToSave);
      const p4Jobs = drinkingWaterJobs.filter(j => j.receiptNumber === receiptToSave);
      makePageUpload('P1', p1Jobs);
      makePageUpload('P2', p2Jobs);
      makePageUpload('P3', p3Jobs);
      makePageUpload('P4', p4Jobs);
    } catch (error: any) {
  ...
  const handleSaveAllDrafts = useCallback(async () => {
    ...
    for (const receipt of uniqueReceipts) {
      try {
        ...
        // ✅ 페이지별 사진 업로드 (P1~P4 독립, userName+comment 포함)
        const makePageUploadAll = (
          pageCode: 'P1' | 'P2' | 'P3' | 'P4',
          jobs: {
            photos?: (({ base64: string; mimeType: string; file: { name: string } }) & { uid?: string })[];
            photoComments?: Record<string, string>;
          ...
          deletePhotosFromServer(receipt, pageCode, userName)
            .catch(() => {})
            .then(() => Promise.all(photoTasks));
        };
        makePageUploadAll('P1', structuralCheckJobs.filter(j => j.receiptNumber === receipt));
        makePageUploadAll('P2', photoLogJobs.filter(j => j.receiptNumber === receipt));
        makePageUploadAll('P3', fieldCountJobs.filter(j => j.receiptNumber === receipt));
        makePageUploadAll('P4', drinkingWaterJobs.filter(j => j.receiptNumber === receipt));
      } catch (err: any) {
  ...
  const handleLoadDraft = useCallback(async (receipt?: string) => {
    const receiptToLoad = receipt || receiptNumber;
    ...
    try {
      ...
      downloadPhotosFromServer(receipt_no, userName).then(serverPhotos => {
        ...
        setDraftMessage({ type: 'success', text: `'${receipt_no}' 데이터+사진(P1:${p1Items.length} P2:${p2Items.length} P3:${p3Items.length} P4:${p4Items.length}장) 불러오기 완료` });
      }).catch(e => console.warn('[사진 불러오기 실패]', e.message));
    } catch (error: any) {
