...
const AnalysisPage: React.FC<AnalysisPageProps> = ({
  ...
  onSaveDraft, onLoadDraft, onSaveAllDrafts, onLoadAllDrafts, draftMessage
}) => {
...
- 날짜가 전혀 없고 시간만 있는 경우 시간만 사용(추출 가능 시).`;
    ...
    if (item === "TN/TP") {
        ...
        if (inspectionStartDate && inspectionEndDate) {
            context += `\n- 검사 기간(선택): ${inspectionStartDate} ~ ${inspectionEndDate} (기간이 있으면 자정 넘어가면 날짜 증가)`;
        }
...
추출 불가하면 [] 를 반환.`;
        ...
        if (inspectionStartDate) {
            context += `\n- 검사 시작일(선택): ${inspectionStartDate}  (시간과 조합)`;
        }
        if (inspectionEndDate) {
            context += `\n- 검사 종료일(선택): ${inspectionEndDate}    (자정 넘어가면 날짜 증가)`;
        }
    ...
    try {
        ...
        const imageProcessingPromises = activeJob.photos.map(async (image, index) => {
            ...
            try {
                ...
                const compressedDataUrl = await compressImage(image.base64, image.mimeType, 1920, 1920, 0.85);
            ...
            <div className="flex gap-2">
              {onSaveAllDrafts && (
                <button
                  ...
                  className="flex items-center gap-1 px-2.5 py-1 bg-slate-600 hover:bg-slate-500 text-white text-xs font-semibold rounded-md transition-colors border border-slate-500"
                >
                  💾 저장
                </button>
              )}
              {onLoadAllDrafts && (
                <button
                  ...
                  className="flex items-center gap-1 px-2.5 py-1 bg-sky-600 hover:bg-sky-500 text-white text-xs font-semibold rounded-md transition-colors"
                >
                  📥 불러오기
                </button>
              )}
