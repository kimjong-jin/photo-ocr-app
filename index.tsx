  ...
  payload.ocrData.forEach((entry) => {
    if (payload.pageType === 'DrinkingWater') {
      ...
      if (entry.identifier && typeof entry.value === 'string' && entry.value.trim()) {
        const isAllowedTn = P3_TN_IDENTIFIERS.includes(entry.identifier);
        const isAllowedTp = payload.item === 'TP' && P3_TP_IDENTIFIERS.includes(entry.identifier);
        
        if (isAllowedTn || isAllowedTp) {
