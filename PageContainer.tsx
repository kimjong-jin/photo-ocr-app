...
const PageContainer: React.FC<PageContainerProps> = ({ userName, userRole, userContact, onLogout }) => {
      ...
      jobsToSaveP3.forEach((job) => {
        ...
        itemsToProcess.forEach((subItem) => {
          ...
          (job.processedOcrData || []).forEach((entry) => {
            ...
            const valueSource = subItem === 'TU' ? entry.value : job.selectedItem === 'TU/CL' ? entry.valueTP : entry.value;
  ...
  const createDrinkingWaterJob = (itemName: string, data: LoadedData): DrinkingWaterJob => {
    ...
    const reconstructedOcrData = DRINKING_WATER_IDENTIFIERS.map((identifier) => {
      const entry: DrinkingWaterJob['processedOcrData'][0] = { id: uuid(), time: '', value: '', identifier };
      if (itemName === 'TU/CL') entry.valueTP = '';
      const tuData = (values as any).TU || {}; const clData = (values as any).Cl || {};
      let pVal: string | undefined, sVal: string | undefined, tVal: string | undefined;
  ...
  const handleLoadDraft = useCallback(async () => {
    ...
    try {
      ...
      const createP1P2Job = (itemName: string): PhotoLogJob => {
        const reconstructedOcrData: PhotoLogJob['processedOcrData'] = [] as any;
        if (itemName === 'TN/TP') {
          const tnData = (values as any).TN || {};
          const tpData = (values as any).TP || {};
          ...
          Object.entries<any>(tpData).forEach(([id, data]) => {
            const key = data.time || id;
            if (!timeToEntryMap[key]) timeToEntryMap[key] = { id: uuid(), time: data.time };
            (timeToEntryMap[key] as any).valueTP = data.val;
            (timeToEntryMap[key] as any).identifierTP = id;
          });
          Object.values(timeToEntryMap)
            .sort((a, b) => (a.time || '').localeCompare(b.time || ''))
            .forEach((partialEntry) => {
              (reconstructedOcrData as any).push({
                id: partialEntry.id!,
          ...
          const itemData: any = (values as any)[itemName] || {};
          Object.entries<any>(itemData)
            .sort(([, a], [, b]) => (a.time || '').localeCompare(b.time || ''))
            .forEach(([id, entryData]) => {
              if (entryData) {
                (reconstructedOcrData as any).push({ id: uuid(), time: entryData.time, value: entryData.val, identifier: id });
              }
            });
        }
        return {
          ...
          photos: [],
          photoComments: {},
          processedOcrData: reconstructedOcrData as any,
      ...
      const newP4Jobs = p4Items
        .map((itemName) => {
          const itemData: any = (values as any)[itemName as MainStructuralItemKey];
  ...
  const handleAddTask = useCallback(() => {
    ...
    if (activePage === 'photoLog' || activePage === 'fieldCount') {
      const newJob: PhotoLogJob = {
        ...
        receiptNumber,
        siteLocation,
        selectedItem: newItemKey as any,
        photos: [],
        photoComments: {},
        processedOcrData: null as any,
        rangeDifferenceResults: null,
        concentrationBoundaries: null,
        decimalPlaces: 0,
        details: '',
      ...
      const newJob: DrinkingWaterJob = {
        ...
        selectedItem: newItemKey as any,
        details: '',
        processedOcrData: initialData as any,
        decimalPlaces: 2,
        photos: [],
        submissionStatus: 'idle',
        submissionMessage: undefined,
      ...
      const newChecklist: Record<string, StructuralCheckSubItemData> = Object.fromEntries(
        CHECKLIST_DEFINITIONS[key].map((itemName) => {
          let defaultNotes = '';
          if (itemName === '정도검사 증명서') {
            defaultNotes = JSON.stringify({ presence: 'not_selected' } as CertificateDetails);
          }
          if (itemName === '측정방법확인') {
            const preferredMethod = (PREFERRED_MEASUREMENT_METHODS as any)[key];
            if (preferredMethod) {
      ...
      if (key === 'TU') (newChecklist as any)['측정범위확인'].notes = '0-10 NTU';
      if (key === 'Cl') (newChecklist as any)['측정범위확인'].notes = '0-2 mg/L';
