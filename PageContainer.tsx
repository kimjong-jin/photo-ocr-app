        ...
        {(
          ...
          <div className="sticky top-0 z-30 w-full max-w-3xl mb-0 bg-slate-900/95 backdrop-blur-md rounded-xl border border-slate-700/60 shadow-md overflow-hidden divide-y divide-slate-700/40">
            <div>
              <button
                ...
                className={`w-full flex justify-between items-center text-left px-4 py-2.5 transition-all hover:bg-slate-800/60 ${
                  openSections.includes('applicationOcr') ? 'bg-slate-800/40' : ''
                }`}
                aria-expanded={openSections.includes('applicationOcr')}
                aria-controls="application-ocr-section"
              >
                <span className="text-xs font-semibold text-slate-300 tracking-wide uppercase">📋 목록</span>
                <ChevronDownIcon
                  className={`w-3.5 h-3.5 text-slate-500 transition-transform ${openSections.includes('applicationOcr') ? 'rotate-180' : ''}`}
                />
              </button>
              <div
            ...
            <div>
              <button
                ...
                className={`w-full flex justify-between items-center text-left px-4 py-2.5 transition-all hover:bg-slate-800/60 ${
                  openSections.includes('addTask') ? 'bg-slate-800/40' : ''
                }`}
                aria-expanded={openSections.includes('addTask')}
                aria-controls="add-task-section"
              >
                <span className="text-xs font-semibold text-slate-300 tracking-wide uppercase">⚙️ 공통 정보 및 작업 관리</span>
                <ChevronDownIcon
                  className={`w-3.5 h-3.5 text-slate-500 transition-transform ${openSections.includes('addTask') ? 'rotate-180' : ''}`}
                />
              </button>
            ...
            <div>
              <button
                ...
                className={`w-full flex justify-between items-center text-left px-4 py-2.5 transition-all hover:bg-slate-800/60 ${
                  openSections.includes('data') ? 'bg-slate-800/40' : ''
                }`}
                aria-expanded={openSections.includes('data')}
                aria-controls="data-section"
              >
                <span className="text-xs font-semibold text-slate-300 tracking-wide uppercase">🗄️ 데이터 관리</span>
                <ChevronDownIcon className={`w-3.5 h-3.5 text-slate-500 transition-transform ${openSections.includes('data') ? 'rotate-180' : ''}`} />
              </button>
          ...
          <div className="w-full max-w-3xl mt-2 mb-4 bg-slate-900/70 rounded-xl border border-slate-700/60 shadow-md overflow-hidden">
            <button
              ...
              className={`w-full flex justify-between items-center text-left px-4 py-2.5 transition-all hover:bg-slate-800/60 ${
                openSections.includes('locationHelper') ? 'bg-slate-800/40' : ''
              }`}
              aria-expanded={openSections.includes('locationHelper')}
              aria-controls="location-helper-section"
            >
              <span className="text-xs font-semibold text-slate-300 tracking-wide uppercase">📍 위치 도우미</span>
              <ChevronDownIcon className={`w-3.5 h-3.5 text-slate-500 transition-transform ${openSections.includes('locationHelper') ? 'rotate-180' : ''}`} />
            </button>
